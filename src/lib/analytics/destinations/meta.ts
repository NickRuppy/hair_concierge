  ...
  track<E extends AppEventName>(eventName: E, payload: AppEventMap[E]) {
    switch (eventName) {
      case "checkout_started": {
        ...
        return trackMetaCheckoutStarted(data.source, data.interval ?? null)
      }
      case "pricing_viewed": {
        const data = payload as AppEventMap["pricing_viewed"]
        return trackMetaPricingViewed(data.source)
      }
      ...
      case "quiz_completed":
        return trackMetaQuizCompleted()
      case "quiz_lead_captured": {
        const data = payload as AppEventMap["quiz_lead_captured"]
        return trackMetaLeadCaptured(data.marketingConsent)
      }
      case "quiz_started": {
        const data = payload as AppEventMap["quiz_started"]
        return trackMetaQuizStarted(data.stepName, data.stepNumber)
      }
