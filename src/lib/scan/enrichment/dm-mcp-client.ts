...
import { parseSearchToonTables, parseToonTable, ToonParseError } from "./toon"
...
/**
 * Shared session bootstrap, timeout budget, error taxonomy and session-expired single retry
 * for every dm MCP tool call. `toolName`/`toolArguments` select the call; `parseResult` turns
 * the tool's `result` string into rows (pipe TOON for getProductDetails, name-prefixed
 * comma TOON for searchProducts).
 */
function callDmMcpTool(
  deps: CreateDmMcpClientDeps,
  toolName: string,
  toolArguments: Record<string, unknown>,
  parseResult: (result: string) => DmProductDetailsRow[],
): Promise<DmProductDetailsRow[]> {
  return (async () => {
    ...
    const operation = async () => {
      for (let attempt = 0; attempt < 2; attempt++) {
        remaining()
        const client = new Client({ name: "chaarlie-scan", version: "1.0.0" }, { capabilities: {} })
        ...
        const exchange = async () => {
          ...
          if (
            ...
            throw new DmMcpError("malformed")
          const rows = parseResult(payload.result)
          remaining()
          return rows
        }
        try {
...
export function createDmMcpClient(deps: CreateDmMcpClientDeps): DmMcpClient {
  return {
    getProductDetails(gtins) {
      return callDmMcpTool(deps, "getProductDetails", { gtins: gtins.map(Number) }, parseToonTable)
    },
    searchProducts(query) {
      return callDmMcpTool(deps, "searchProducts", { query }, parseSearchToonTables)
    },
  }
}
