...
function currentLegacyAnswers(profile: Record<string, unknown>): QuizAnswers {
  ...
  return {
    ...
    treatment: (profile.chemical_treatment as string[]).map(
      (value) =>
        lookup(value, {
          natural: "natur",
          colored: "gefaerbt",
...
function rebaseSource(source: SupportedStage1Source, current: QuizAnswers): SupportedStage1Source {
  const fresh = buildLegacyQuizStage1Source({
    leadId: source.kind === "legacy_quiz" ? source.leadId : "profile",
    answers: current,
  })
  const projected = normalizedBasics(
    adaptPersonalPlanAnswersForOffer(source.answers as never).answers,
  ) as Record<string, unknown>
  const actual = normalizedBasics(current) as Record<string, unknown>
...
export function prepareScannerContext(read: ScannerSourceRead): PreparedScannerContext | null {
  ...
  const initial = validNeed(read, read.initial, "initial")
  const refined = validNeed(read, read.refined, "refined")
  ...
  const base = computeNeedPlan({
    rawEnvelope: source,
    artifactId:
      source.kind === "legacy_quiz" ? source.leadId : (read.initial?.id ?? read.refined!.id),
