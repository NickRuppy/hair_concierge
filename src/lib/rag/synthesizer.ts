...
function appendFollowupQuestions(profileText: string, followupQuestions?: string[]): string {
  ...
  result +=
    "\n\n(HINWEIS: Gib eine erste Produktempfehlung basierend auf dem vorhandenen Kontext. Stelle dabei auch diese Rückfragen, um die Empfehlung im nächsten Schritt zu verfeinern:"
  for (const q of followupQuestions) {
    result += `\n- ${q}`
  }
  result +=
    "\nFormuliere die Empfehlung als vorsichtige erste Einschätzung, nicht als finale Antwort.)"
  return result
}
...
function formatUserProfile(
  ...
  if ((profile.goals?.length ?? 0) === 0 && profile.desired_volume) {
    parts.push(
      `Gewünschtes Volumen: ${DESIRED_VOLUME_LABELS[profile.desired_volume] ?? profile.desired_volume}`,
    )
  }
  ...
  if (profile.additional_notes) {
    parts.push(`Zusätzliche Infos: ${profile.additional_notes}`)
  }
  ...
  if (clarificationQuestions && clarificationQuestions.length > 0) {
    result +=
      "\n\n(HINWEIS: Stelle zuerst gezielte Rückfragen, um die Situation zu verstehen. Nenne dabei KEINE konkreten Produktnamen — auch nicht die Produkte aus der Datenbank unten. Produktempfehlungen kommen erst, wenn du genug Kontext hast."
    result +=
      "\n\nStelle insbesondere diese Fragen (in deinem eigenen Stil, nicht wörtlich kopieren):"
...
function formatShampooProfile(
  ...
  if (clarificationQuestions && clarificationQuestions.length > 0) {
    result +=
      "\n\n(HINWEIS: Shampoo-Klärungsrunde. Stelle AUSSCHLIESSLICH Rückfragen zu den fehlenden Shampoo-Feldern."
    result +=
    ...
    if (clarificationQuestions.length === 1) {
      result += "\nStelle genau diese eine Rückfrage:"
    } else {
      result += "\nStelle genau diese Rückfragen:"
    }
    for (const q of clarificationQuestions) {
      result += `\n- ${q}`
    }
...
function formatRagContext(chunks: ContentChunk[]): string {
  if (chunks.length === 0) {
    return "Keine zusätzlichen Informationen aus der Wissensbasis verfügbar."
  }
...
const PRODUCT_SECTION_HEADERS: Record<string, string> = {
  ...
  oil: "Passende Öle aus unserer Datenbank",
...
const ENGINE_ACTION_LABELS = {
  add: "neu einführen",
  replace: "ersetzen",
  increase_frequency: "häufiger nutzen",
  decrease_frequency: "seltener nutzen",
  keep: "beibehalten",
  remove: "entfernen",
  behavior_change_only: "vor allem über Verhalten lösen",
} as const
...
const PEELING_TYPE_LABELS = {
  acid_serum: "säurebasiertes Peeling",
  ...
  switch (categoryDecision.category) {
    case "shampoo":
      ...
      if (categoryDecision.targetProfile?.secondaryBucket) {
        parts.push(
          `- Sekundär-Bucket: ${SHAMPOO_BUCKET_LABELS[categoryDecision.targetProfile.secondaryBucket] ?? categoryDecision.targetProfile.secondaryBucket}`,
        )
      }
    ...
    case "mask":
      ...
      parts.push(`- Masken-Stärke: ${categoryDecision.targetProfile?.needStrength ?? 0}`)
      break
    case "leave_in":
    ...
    case "oil":
      if (categoryDecision.targetProfile?.purpose) {
        parts.push(`- Öl-Zweck: ${OIL_USE_MODE_LABELS[categoryDecision.targetProfile.purpose]}`)
      }
      if (categoryDecision.targetProfile?.matcherSubtype) {
        parts.push(`- Öl-Typ: ${OIL_SUBTYPE_LABELS[categoryDecision.targetProfile.matcherSubtype]}`)
      }
      parts.push(`- Klärung nötig: ${categoryDecision.clarificationNeeded ? "ja" : "nein"}`)
      if (categoryDecision.noRecommendationReason) {
        parts.push(
          `- Kein Öl empfohlen: ${OIL_NO_RECOMMENDATION_LABELS[categoryDecision.noRecommendationReason]}`,
        )
      }
      break
    case "bondbuilder":
      if (categoryDecision.targetProfile?.bondRepairIntensity) {
        parts.push(
          `- Bond-Intensität: ${BOND_REPAIR_LABELS[categoryDecision.targetProfile.bondRepairIntensity]}`,
        )
      }
      ...
      if (categoryDecision.targetProfile?.peptideChainLane) {
        parts.push("- Lane-Hinweis: Peptid-/Längsstruktur-Produkte plausibel")
      }
      if (categoryDecision.targetProfile?.mixedOrSevereCombo) {
        parts.push("- Schwere/Mix-Lage: bis zu drei reguläre Bondbuilder-Karten erlaubt")
      }
      break
...
function formatRoutinePlan(routinePlan?: RoutinePlan): string {
  ...
  if (routinePlan.compare_cwc_owc) {
    parts.push(
      "- Vergleichsmodus: Erkläre CWC und OWC erst kurz gegeneinander und entscheide dich dann für die passendere Variante für dieses Profil.",
    )
  }
  ...
  parts.push(
    hasWashProtectionTechnique
      ? "- Niveau: Hohe Ebene für alle Slots; nur bei CWC/OWC sind kompakte nummerierte Wash-Day-Schritte erlaubt."
  ...
  for (const section of routinePlan.sections) {
    ...
    for (const slot of section.slots) {
      ...
      if ((slot.attached_products ?? []).length > 0) {
        parts.push(
          `  Angehängte Produkte: ${(slot.attached_products ?? [])
            .map((product) => {
  ...
  parts.push(
    "\nWICHTIG: Folge dieser Struktur. Erkläre pro Slot erst die Routine-Logik und den Fit zum Profil. Nenne bereits angehängte Produkte erst danach und nur direkt beim passenden Slot.",
  )
...
function formatProducts(
  ...
  if (products.length === 0) {
    if (clarificationQuestions && clarificationQuestions.length > 0) {
      return `${categoryDecisionBlock}\n\nWICHTIG: Stelle nur die fehlenden Rückfragen, nenne keine Produkte und behandle das nicht als Katalog-No-Match.`
    }
    ...
    if (
      ...
      return `${categoryDecisionBlock}\n\nWICHTIG: Sage klar, dass aktuell keine Maske nötig ist. Nenne in diesem Fall KEINE konkreten Maskenprodukte.`
    }
    ...
    if (
      ...
      return `${categoryDecisionBlock}\n\nWICHTIG: Sage klar, dass aktuell kein Öl empfohlen wird. Begründe das nur mit dem Engine-Entscheidungsblock. Nenne KEINE konkreten Öle und improvisiere keinen Therapie-Öl-Ersatz.`
    }

    if (supportCategoryNoRecommendationLabel) {
      return `${categoryDecisionBlock}\n\nWICHTIG: Sage klar, dass aktuell ${supportCategoryNoRecommendationLabel} empfohlen ist. Begründe das nur mit dem Engine-Entscheidungsblock. Nenne in diesem Fall KEINE konkreten Produkte und behandle das nicht als Katalog-No-Match.`
    }
    ...
    .map((p) => {
      ...
      if (p.recommendation_meta) {
        ...
        if (meta.category === "mask") {
          ...
          parts.push(`  Stärke: ${strengthLabel}`)
        ...
        if (meta.category === "oil") {
          ...
          if (meta.matched_subtype) {
            parts.push(`  Öl-Typ: ${OIL_SUBTYPE_LABELS[meta.matched_subtype]}`)
          }
          if (meta.use_mode) {
            parts.push(`  Anwendung: ${OIL_USE_MODE_LABELS[meta.use_mode]}`)
          }
          if (meta.adjunct_scalp_support) {
            parts.push("  Kopfhaut-Hinweis: nur als unterstützende Zusatzpflege")
          }
        }
  ...
  return `${categoryDecisionBlock}\n\n${header}:\n${productList}\n\nWICHTIG: Verwende die EXAKTEN Produktnamen (wie oben geschrieben) wenn du sie erwähnst — die Namen werden in der App als klickbare Links dargestellt.`
}
...
1. Nutze für die Shampoo-Begründung NUR diese Signale: Haardicke, Kopfhaut-Typ, Kopfhaut-Beschwerden, den abgeleiteten Shampoo-Bucket und den Shampoo-Entscheidungsblock.
...
4. Wenn Kopfhaut-Beschwerden vorliegen, erkläre klar, dass dieser aktuelle Zustand den Shampoo-Bucket vorübergehend priorisiert. Wenn keine Beschwerden vorliegen, richte den Bucket am Kopfhaut-Typ aus.
5. Erkläre ZUERST, welche Shampoo-Eigenschaften ideal für dieses Nutzerprofil sind. Empfehle DANN konkrete Produkte und erkläre WARUM jedes Produkt zu genau diesem Profil passt.
6. Begründe Shampoo-Fit NICHT mit Haarstruktur, Zielen, chemischer Behandlung, Waschfrequenz oder anderen Randprofilen.
7. Wenn die Kopfhaut-Beschwerden "trockene Schuppen" sind, sage kurz, dass wir hier den trockenen Shampoo-Bucket nutzen. Ergänze: "Wenn die Schüppchen nach 4-6 Wochen nicht besser werden, empfehlen wir einen Dermatologen aufzusuchen."
8. Erwähne Haarmuster wie glatt, wellig, lockig oder coily NICHT als Shampoo-Fit-Signal, auch wenn diese Infos im Nutzerprofil stehen.
9. Wenn ZWEI Shampoo-Buckets empfohlen werden (Behandlung + Basis), erkläre die Rotation: Anti-Schuppen-Shampoo 2-3x pro Woche, an den anderen Waschtagen das Basis-Shampoo. Betone, dass Seborrhoische Dermatitis chronisch ist und das Anti-Schuppen-Shampoo langfristig als Erhaltung (1-2x/Woche) nötig bleiben kann.`,
  conditioner: `

## Conditioner-Empfehlungen:
Wenn du Conditioner-Empfehlungen gibst:
1. Nutze für die Conditioner-Begründung NUR den Conditioner-Entscheidungsblock und die Produkt-Metadaten.
...
4. Erkläre ZUERST, was das Haar laut Zugtest gerade braucht: Protein, Feuchtigkeit oder ausgewogene Pflege.
5. Beziehe DANN das erwartete Gewicht und den Reparaturbedarf ein. Erkläre, ob der Conditioner eher leicht, mittel oder reichhaltig sein sollte und wie viel Repair-Level sinnvoll ist.
6. Empfehle DANN konkrete Produkte und erkläre WARUM jedes Produkt sowohl zum Pflegefokus als auch zu Gewicht und Repair-Level passt.`,
  leave_in: `

## Leave-in-Empfehlungen:
Wenn du Leave-in-Empfehlungen gibst:
1. Nutze für Leave-in-Begründungen NUR den Leave-in-Entscheidungsblock und die Produkt-Metadaten.
...
4. Erkläre zuerst, was das Leave-in leisten soll: Pflegefokus, Styling-Kontext und erwartetes Gewicht.
5. Unterscheide IMMER sauber zwischen "Conditioner-Ersatz möglich" und "nur zusätzlicher Booster". Sage bei Booster-Profilen niemals, dass das Leave-in den Conditioner ersetzt.
6. Empfehle dann konkrete Produkte und erkläre WARUM jedes Produkt genau zu Pflegefokus, Styling-Kontext und Conditioner-Rolle passt.`,
  oil: `

## Öl-Empfehlungen:
Wenn du Öl-Empfehlungen gibst:
1. Nutze für Öl-Begründungen NUR den Öl-Entscheidungsblock und die Produkt-Metadaten.
2. Wenn im Öl-Entscheidungsblock Profilfelder fehlen, frage NUR nach Haardicke und Öl-Zweck. Nenne keine Produkte.
3. Wenn der Öl-Entscheidungsblock sagt, dass aktuell kein Öl empfohlen wird, sage das klar und nenne keine ausweichenden Öle oder DIY-Mischungen.
4. Wenn der Öl-Entscheidungsblock sagt, dass es keinen exakten Katalog-Match gibt, sage das klar und nenne keine Öle aus anderen Öl-Typen oder anderen Haardicken.
5. Erkläre zuerst, welche Art Öl hier gemeint ist: Hair Oiling vor dem Waschen, Styling-Finish oder leichtes Trocken-Öl.
6. Begründe den Fit danach nur über Öl-Typ, Haardicke und die Anwendungslogik aus den Metadaten.
7. Wenn Kopfhautthemen mitlaufen, ordne natürliche Öle nur als unterstützende Zusatzpflege ein. Stelle sie NICHT als primären Behandlungsweg für Schuppen, gereizte Kopfhaut oder Haarwachstum dar.
8. Wenn der Nutzer gezielt ein Therapie-Öl oder eine spezifische Ölmischung sucht und diese nicht im Katalog ist, sage das ehrlich statt einen Ersatz zu erfinden.`,
  routine: `

## Routine-Antworten:
Wenn ein Routine-Plan vorhanden ist:
1. Nutze den Routine-Plan als primären Rahmen der Antwort.
2. Bleibe bewusst auf hoher Ebene: Kombinationen, Frequenz, Rollen der Schritte und das Warum dahinter. Ausnahme: Bei aktiven CWC/OWC-Slots darfst du die kompakten nummerierten Wash-Day-Schritte aus dem Routine-Plan übernehmen.
3. Erkläre die Slots gemäß ihrer Aktion: keep = bestätigen, adjust = Frequenz/Fokus anpassen, add = neu einführen, upgrade = gleicher Slot aber gezielterer Fokus, avoid = für jetzt eher nicht priorisieren.
4. Starte mit den Bausteinen, die schon gut sitzen oder nur leicht angepasst werden sollten. Erkläre danach, was fehlt oder gezielter werden darf.
5. Begründe pro relevantem Slot erst kurz den Fit zum Profil, also über Haarmuster, Ziele, Probleme oder Routinekontext, bevor du ein Produkt nennst.
6. Wenn Produkte angehängt sind, nenne nur diese Produkte und ordne sie direkt dem gerade erklärten Slot im selben Absatz oder Bullet zu.
7. Wenn keine Produkte angehängt sind, bleibe bei Kategorien und Routine-Logik statt konkrete Produkte zu improvisieren.
8. Erfinde keine zusätzlichen Kategorien, Schritte oder Produkttypen außerhalb des Routine-Plans.
9. Detaillierte Anwendungstechniken gehören NICHT in diese Antwort, außer die kompakten nummerierten CWC/OWC-Schritte stehen bereits im Routine-Plan.
10. Bei Kopfhautthemen bleibe konservativ und nicht-medizinisch.
11. Wenn Bond Builder relevant ist: Olaplex No. 0+3, K18 Molecular Repair Leave-in und Epres gehören zu den wenigen Produkten mit nachgewiesener Bond-Technologie. Viele Produkte mit "Bond" im Namen pflegen nur die Oberfläche, reparieren aber nicht die innere Haarstruktur. Das sind Technologie-Beispiele, keine Produktempfehlungen.
12. Nenne bei Bond Builder den Unterschied zwischen Längs- und Querverbindungen nur, wenn der Routine-Plan das vorgibt. Erfinde keine eigene K18-vs-Olaplex-Logik.
13. Wenn Tiefenreinigung aktiv ist, unterscheide sauber zwischen Kopfhaut-Tiefenreinigung, Haar-Reset und Hard Reset. Erkläre nur die Slots, die im Routine-Plan wirklich aktiv sind.
14. Stelle Tiefenreinigung nie als universellen Pflichtschritt, tägliche Empfehlung oder "Detox für alle" dar. Sie ist immer bedarfsgetrieben.
15. Wenn Tiefenreinigung für die Längen aktiv ist, erwähne die Anschluss-Pflege klar: danach Conditioner oder Maske.
16. Bei Schuppen, deutlichem Juckreiz oder gereizter Kopfhaut bleibe extra konservativ und rahme Tiefenreinigung nicht als primären Lösungsweg; erfinde keine medizinischen Aussagen.
...
18. Wenn trockene Kopfhaut als Gegenpol erwähnt wird, ordne Hair Oiling höchstens als sanfte Zusatzpflege ein - nie als eigentlichen Reinigungsweg.
19. Wenn der Routine-Plan Vergleichsmodus für CWC/OWC signalisiert, erkläre erst kurz den Unterschied beider Methoden, wähle dann die passendere Option für dieses Profil und führe nur mit dieser Variante weiter.
20. Wenn Bürsten & Tools relevant sind, bleibe bei Funktionslogik und Sicherheitsregeln: Slip beim Entwirren, von unten nach oben arbeiten, sanfter Druck auf der Kopfhaut und keine improvisierten Marken- oder Tool-Empfehlungen.`,
  mask: `

## Masken-Empfehlungen:
Wenn du Masken-Empfehlungen gibst:
1. Behandle Masken als Zusatzpflege, nicht als Conditioner-Ersatz.
2. Betone Anwendung auf Längen/Spitzen (nicht Kopfhaut).
3. Erkläre klar die Reihenfolge: Shampoo -> Maske -> Conditioner.
4. Nutze den "Masken-Entscheidung", "Stärke", "Typ" und "Anwendung"-Kontext aktiv.
5. Wenn die Masken-Entscheidung sagt, dass aktuell keine Maske nötig ist, sage das klar und nenne keine Maskenprodukte.
6. Sage niemals, dass Masken Schäden "vorbeugen" oder "verhindern". Masken pflegen und erhalten den aktuellen Zustand — sie schützen nicht präventiv vor zukünftigen Schäden.`,
}
