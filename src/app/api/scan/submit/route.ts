...
import {
  ...
  scanProductIntakeIdentifierSchema,
...
const submitBodySchema = z
  .object({
    identifier: scanProductIntakeIdentifierSchema,
 ...
 * `submitScanProductIntake` never reads/writes `user_product_usage`, so this value is
 * inert for the scan path — stored on `product_submissions.frequency_range` only because
 * the column is `NOT NULL` for every source. Task 4's report flagged this as a copy/UX
 * call left open for whoever wires the route; picked the schema's own median value rather
 * than invent a fake "unknown" sentinel the enum doesn't have.
 */
const SCAN_SUBMIT_PLACEHOLDER_FREQUENCY: ProductFrequency = "weekly_2x"
...
export function createScanSubmitRouteHandler(deps: ScanSubmitRouteDeps) {
  return async function POST(request: Request) {
    ...
    const input: ScanProductIntakeSubmissionInput = {
      ...
      frequency_range: SCAN_SUBMIT_PLACEHOLDER_FREQUENCY,
