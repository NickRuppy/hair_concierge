 ...
 *
 *   200 { status: { kind, label, canStartResearch } }  — the item's status after the write
 */
...
export function createDiscoveryResearchHandler(overrides: DiscoveryResearchRouteDependencies = {}) {
  ...
  return async function POST(
    ...
    try {
      if (action.type === "enqueue") {
        ...
        if (created.kind === "already_in_catalog") {
          ...
          // way the checklist would have stored it — only if the row is still product-less.
          await assignCatalogProduct(admin, {
            intakeId: intake.id,
            itemId: before.item.id,
            productId: created.productId,
          })
        } else {
          await attachSubmission(admin, {
            intakeId: intake.id,
            itemId: before.item.id,
    ...
    try {
      ...
      return discoveryCockpitJson({ status: statusBody(after.status) })
    } catch (error) {
      console.error("[discovery] research state re-read failed:", error)
      // The write happened; only the fresh status could not be read.
      return discoveryCockpitJson({
        status: {
