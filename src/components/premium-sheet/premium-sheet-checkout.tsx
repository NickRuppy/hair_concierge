 ...
 *
 * Two things separate this from the offer page's `payment-method-checkout.tsx` mount:
 *
 ...
 *
 * `EmbeddedCheckoutProvider` calls `fetchClientSecret` once per mount, so the `key` the
 * sheet gives this component (the attempt id) is what starts a NEW checkout; re-rendering
 * never does.
 */
export function PremiumSheetCheckout({
  ...
  // re-render and remount the provider mid-payment.
  ...
  const onComplete = useCallback(() => {
    ...
    // No session id means the creation response was malformed — there is nothing the server
    // could verify, so this is a failure, not a silent success.
    if (!sessionId) {
      onFailed()
      return
    }
