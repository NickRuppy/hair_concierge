...
2. **Naming / Branding der App:** Soll der Launch an eine bestehende Dachmarke andocken oder einen komplett eigenstaendigen Namen tragen?
...
Keine technische Recherche noetig — reine Markenfrage fuer Domain- und Brand-Review.

| Option | Beschreibung |
|--------|-------------|
| **A: Erweiterung einer bestehenden Dachmarke** | Fuegt sich sichtbar in eine bestehende Markenfamilie ein. Klar, einpraegsam, direkt beim Thema. |
| **B: "Hair Concierge" mit Dachmarken-Absender** | Nutzt einen bestehenden Markennamen als Absender, gibt der App aber einen eigenstaendigen Namen. |
| **C: Komplett eigenständiger Name** | Unabhaengig von einer bestehenden Markenfamilie. Mehr Freiheit, aber weniger Synergien. |
...
|--------|-------------|------------|
| **A: Bestätigen, dass Density reicht** | Wir haben es bereits — nur noch fachlich klaeren, ob es auch fuer Masken/Shampoo relevant ist. | Ggf. `density` auch in den Mask-Reranker einbauen (Weight-Fit pro Density). |
...
**Vorschlag:** Option A — trockene Schueppchen sind meistens ein Symptom trockener Kopfhaut, nicht von Schuppen (Malassezia). Aber unbedingt fachlich bestaetigen lassen, weil die Behandlung sich unterscheidet.
...
**Vorschlag:** Option B ist bei Dermatologen ueblich (Rotation schont die Kopfhaut), aber das muss fachlich bestaetigt werden. Falls zu komplex → Option A mit Chat-Hinweis als Zwischenloesung.
...
**Fachliche Rueckmeldung:** "Nix"-Masken sind fuer allgemeine Pflege-Benefits: bessere Kaemmbarkeit, Schutz gegen Stress, mehr Glanz. Nicht an einen spezifischen Mangel gebunden — eher praeventive/allgemeine Wellness-Masken.
...
**Fachliche Rueckmeldung:** Mehr Belastungsfaktoren → reichhaltigere/fetthaltigere Masken. Hitze-Styling ist weniger belastend als Protein-/Feuchtigkeitsmangel. Die drei Faktoren (Hitze, Protein/Feuchtigkeitsmangel, chemische Behandlung) bilden ein "Dreieck" — sie interagieren miteinander und lassen sich nicht isoliert bewerten. Gutes Profiling ist der Schluessel, damit man ein breites Array an Masken anbieten kann statt immer dieselbe.
...
**Fachliche Rueckmeldung:** Nach Preis sortieren — guenstigstes Produkt zuerst, teuerstes zuletzt.
...
**Fachliche Rueckmeldung:** Noch keine Produktliste vorhanden, kommt bald.

**Entscheidung:** Option B (nur im Chat als Wissensantwort) bis die Produkttabelle vorliegt. Dann auf Option A (eigene Pipeline) umsteigen.

---

### Frage 7: Mechanische Belastung im Onboarding ✅

**Fachliche Rueckmeldung:** Ja, soll abgefragt werden.
...
**Fachliche Rueckmeldung:** "Curl" im Produktnamen bedeutet nicht, dass es nur fuer lockiges Haar ist. `hair_texture` kann bei Conditioner- und Leave-in-Empfehlungen ignoriert werden.
