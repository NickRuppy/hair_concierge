...
export type LeaveInProductionAdapterCliArgs = {
  input: string
  output: string
  overwrite: boolean
}
...
export function parseLeaveInProductionAdapterCliArgs(
  argv: string[],
): LeaveInProductionAdapterCliArgs {
  const parsed: Partial<LeaveInProductionAdapterCliArgs> = { overwrite: false }
...
export function runLeaveInProductionAdapterCli(args: LeaveInProductionAdapterCliArgs): {
  status: LeaveInProductionAdapterOutcome["status"]
} {
  ...
  try {
    ...
    throw new Error(
      `Leave-In research input is unreadable or malformed: ${inputPath} (${error instanceof Error ? error.message : String(error)})`,
    )
  }
  const parsed = leaveInResearchEnvelopeSchema.safeParse(raw)
  const outcome = projectLeaveInForProduction(raw)
  if (!parsed.success && outcome.status === "needs_research") {
    throw new Error(`Invalid Leave-In research envelope: ${outcome.reasons.join("; ")}`)
  }
  ...
  writeOutputAtomically(args.output, args.overwrite, (staging) => {
    ...
    writeFileSync(
      path.join(staging, "projection-summary.md"),
      renderLeaveInProductionMarkdown(outcome),
    )
  })
...
function main() {
  const result = runLeaveInProductionAdapterCli(
    parseLeaveInProductionAdapterCliArgs(process.argv.slice(2)),
  )
