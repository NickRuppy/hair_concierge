/**
 * Narrow, reversible migration for the personal-plan offer v3 reorder.
...
import { personalPlanOfferV3Dashboard } from "../analytics/personal-plan-offer-v3-dashboard"
...
const beforeFingerprints: Record<number, string> = {
  ...
  5250265: "d30d4246c52a25b1f28e1540d22eebc6222ab2a9a0a078bb914cf5d11586133b",
...
const b2V2Counts = `    uniqIf(session_id, section_id = 'hero') AS o1,
    ...
    uniqIf(session_id, section_id = 'personal_plan_complete_plan') AS o3,
    uniqIf(session_id, section_id = 'personal_plan_method') AS o4,
    uniqIf(session_id, section_id = 'personal_plan_before_after') AS o5,
...
const b2V3Counts = `    uniqIf(session_id, section_id = 'hero') AS o1,
    uniqIf(session_id, section_id = 'personal_plan_diagnosis') AS o2,
...
const b2V2Steps = `  SELECT 1 AS sort, '01 Einstieg' AS abschnitt, 'hero' AS section_id, o1 AS sessions, o1 AS vorherige_sessions FROM counts
  ...
  UNION ALL
  SELECT 3 AS sort, '03 Vollständiger Plan' AS abschnitt, 'personal_plan_complete_plan' AS section_id, o3 AS sessions, o2 AS vorherige_sessions FROM counts
  UNION ALL
  SELECT 4 AS sort, '04 So funktioniert der Plan' AS abschnitt, 'personal_plan_method' AS section_id, o4 AS sessions, o3 AS vorherige_sessions FROM counts
  UNION ALL
  SELECT 5 AS sort, '05 Vorher und nachher' AS abschnitt, 'personal_plan_before_after' AS section_id, o5 AS sessions, o4 AS vorherige_sessions FROM counts
  UNION ALL
...
const b2V3Steps = `  SELECT 1 AS sort, '01 Einstieg' AS abschnitt, 'hero' AS section_id, o1 AS sessions, o1 AS vorherige_sessions FROM counts
  UNION ALL
  SELECT 2 AS sort, '02 Persönliche Diagnose' AS abschnitt, 'personal_plan_diagnosis' AS section_id, o2 AS sessions, o1 AS vorherige_sessions FROM counts
  UNION ALL
...
function transformB2(insight: Insight) {
  ...
  if (!query.includes(v2)) {
    if (query.includes(v3)) return clone(insight)
    throw new Error(`Insight ${insight.id} is missing ${v2}.`)
  }
  return withQuery(
    insight,
    replaceExact(
    ...
    (insight.description ?? "").replaceAll(v2, v3),
  )
}
...
export function transformInsight(insight: Insight): Insight {
  switch (insight.id as ResourceId) {
    case 5235347:
      return withDashboardSpec(insight, personalPlanOfferV3Dashboard.insights.o1)
    case 5235348:
      return withDashboardSpec(insight, personalPlanOfferV3Dashboard.insights.o2)
    case 5235350:
      return withDashboardSpec(insight, personalPlanOfferV3Dashboard.insights.o3)
    case 5245339:
      return withDashboardSpec(insight, personalPlanOfferV3Dashboard.insights.o5)
    case 5250265:
      return withDashboardSpec(insight, personalPlanOfferV3Dashboard.insights.o6)
    case 5233190:
    ...
    default:
      throw new Error(`Insight ${insight.id} is not in the approved v3 migration.`)
  }
}
...
function classify(insight: Insight, before: Record<number, string>, after: Record<number, string>) {
  const fingerprint = fingerprintInsight(insight)
  if (fingerprint === after[insight.id]) return "after" as const
  if (fingerprint === before[insight.id]) return "before" as const
  throw new Error(
    `Insight ${insight.id} is neither reviewed v2 state nor expected v3 state (got ${fingerprint}). Refusing to write.`,
  )
}
...
async function writeBackup(path: string, insights: Insight[], now: Date) {
  ...
  const data = {
    schema: "personal-plan-offer-v3-dashboard-migration/v1",
    projectId,
...
async function readBackup(path: string) {
  ...
  if (
    data.schema !== "personal-plan-offer-v3-dashboard-migration/v1" ||
  ...
  if (
    ...
    throw new Error("Backup must contain exactly the eight reviewed v3 resources.")
  }
  return insights
}
...
async function annotate(deps: MigrationDependencies, at: string, deploymentSha: string) {
  ...
  await request(deps, `${apiOrigin}/api/projects/${projectId}/annotations/`, {
    ...
    body: JSON.stringify({
      date_marker: at,
      creation_type: "GIT",
      content: `Personal-plan offer v3 deployed at ${deploymentSha}: pricing moved to section index 2.`,
    }),
  })
...
export async function runMigration(argv: string[], overrides: Partial<MigrationDependencies> = {}) {
  ...
  if (options.restore) {
    ...
    for (const insight of current) {
      if (states.get(insight.id) === "before" || before[insight.id] === after[insight.id]) {
        continue
      }
      const target = backup.find((candidate) => candidate.id === insight.id)!
      await patchAndVerify(deps, target, before[insight.id])
    }
    ...
    deps.output("restored all eight reviewed resources to the backed-up v2 state.")
  ...
  for (const target of targets) {
    ...
    if (actual !== after[target.id]) {
      throw new Error(
        `Insight ${target.id} target drifted from reviewed v3 state (expected ${after[target.id]}, got ${actual}). Refusing to patch.`,
      )
    }
    deps.output(
      `insight ${target.id}: ${states.get(target.id) === "after" ? "already applied" : `${before[target.id]} -> ${after[target.id]}`}`,
    )
  }
  ...
  const pending = current.filter(
    (insight) => states.get(insight.id) === "before" && before[insight.id] !== after[insight.id],
  )
  if (pending.length === 0) {
    if (options.annotationAt) {
      await annotate(deps, options.annotationAt, options.deploymentSha!)
    }
    deps.output("all reviewed v3 resources are already applied; no insight patch performed.")
  ...
  const mixed = current.some(
    (insight) => states.get(insight.id) === "after" && before[insight.id] !== after[insight.id],
  )
  if (mixed) {
  ...
  for (const insight of pending) {
    ...
    await patchAndVerify(deps, target, after[insight.id])
  }
  const reread = await fetchInsights(deps)
  for (const insight of reread) {
    if (fingerprintInsight(insight) !== after[insight.id]) {
