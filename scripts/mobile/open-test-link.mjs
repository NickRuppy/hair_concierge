...
const previousMail = await fetch("http://127.0.0.1:54324/api/v1/messages").then((r) => r.json())
const previousIds = new Set((previousMail.messages || []).map((item) => item.ID))
const started = await fetch("http://127.0.0.1:3218/api/mobile/v1/auth/start", {
  method: "POST",
