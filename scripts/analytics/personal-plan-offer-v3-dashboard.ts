...
const attributionGuardrailQuery = `WITH offer_views AS (
  SELECT
    countIf(notEmpty(ifNull(toString(properties.funnel_session_id), '')) AND properties.funnel_package_key = 'meta_personal_plan_v1' AND properties.offer_revision = 'personal_plan_v3' AND properties.offer_variant IN (${currentVariantsSql})) AS current_page,
    countIf(notEmpty(ifNull(toString(properties.funnel_session_id), '')) AND properties.funnel_package_key = 'meta_personal_plan_v1' AND properties.offer_revision IN ('personal_plan_v1', 'personal_plan_v2') AND properties.offer_variant = 'personal-plan-v1') AS historical_page,
    ...
    countIf(notEmpty(ifNull(toString(properties.funnel_session_id), '')) AND notEmpty(ifNull(toString(properties.funnel_package_key), '')) AND NOT ((properties.funnel_package_key = 'meta_personal_plan_v1' AND properties.offer_revision IN ('personal_plan_v1', 'personal_plan_v2') AND properties.offer_variant = 'personal-plan-v1') OR (properties.funnel_package_key = 'meta_personal_plan_v1' AND properties.offer_revision = 'personal_plan_v3' AND properties.offer_variant IN (${currentVariantsSql})))) AS unexpected_experience,
    count() AS personal_plan_offer_views
  FROM events
  WHERE timestamp >= {filters.dateRange.from} AND timestamp <= {filters.dateRange.to}
    AND event = 'offer_viewed'
    AND ${excludeInternalQa}
    AND (properties.funnel_package_key = 'meta_personal_plan_v1' OR properties.offer_revision IN ('personal_plan_v1', 'personal_plan_v2', 'personal_plan_v3') OR properties.offer_variant IN (${currentVariantsSql}))
), metrics AS (
