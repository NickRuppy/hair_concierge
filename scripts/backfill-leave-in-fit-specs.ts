...
async function main() {
  ...
  const rows: LeaveInFitRow[] = (data ?? []).map((row) => {
  ...
  const { error: upsertError } = await supabase.from("product_leave_in_fit_specs").upsert(
    rows.map(({ product_id, weight, conditioner_relationship, care_benefits }) => ({
      product_id,
      weight,
  ...
  console.log(`Upserted ${rows.length} leave-in fit rows.`)
