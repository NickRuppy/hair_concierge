...
Before implementing a non-trivial plan, any user-facing change, or clearly bounded non-trivial non-user-facing work without a durable plan, present a compact decision-coverage checkpoint with status `pending` or `confirmed` and the buckets `Confirmed with Nick`, `Inherited from evidence or contract`, `Implementation defaults`, and `Open consequential assumptions`. A choice is consequential when choosing differently could change user-visible behavior, product semantics, scope, data ownership, access or payment, rollout, recoverability, or material risk. Surface every such assumption for confirmation; do not interrupt Nick for routine implementation details with no product consequence. Coverage is `confirmed` only when Nick has seen the current record, every consequential choice affecting the handed-off scope is settled, Nick explicitly acknowledges each remaining choice parked with its affected work out of scope, and the record states `Undiscussed consequential assumptions affecting this handoff: none`. Quick audits, questions, queue/status passes, tiny non-user-facing fixes, and routine non-user-facing automation runs may skip the options comparison and decision-coverage checkpoint unless evidence exposes a consequential choice. "Tiny" means mechanically bounded work that cannot affect any consequential category above; line count alone does not make a change tiny.
