...
- `plan-hardening-loop` owns non-trivial planning, decision coverage, meaningful option comparison, user-facing evidence, counterpart plan review, revision, and concrete user-journey alignment when needed. It invokes `prototype` only when a runnable UI or logic artifact is needed to settle a named decision that lighter mockups cannot answer. Use the Working together and planning decisions contract below for authorization and user-facing evidence. Review-only requests end with findings and missing gates, without requiring implementation approval.
- `implementation-loop` owns execution of an approved plan or clearly bounded non-trivial non-user-facing change. It carries settled decisions into execution, then invokes `ready-check` and `request-code-review` before its review-ready handoff; do not rerun them as separate top-level phases on unchanged content.
...
## Working together and planning decisions

Continue authorized work until the requested outcome is complete or a real blocker needs Nick. Carry it through implementation, relevant verification, review, and fixes; workflow stages are not automatic stopping points. Use judgment for routine details and preserve approval already given. If blocked, explain what is missing, recommend the next action, and continue independent authorized work. Publication and production actions retain the explicit boundaries above.

Align with Nick on unresolved choices that materially change product behavior, UX, architecture, scope, data ownership, access/payment, rollout, recoverability, or risk. Present meaningful alternatives and a recommendation when useful. An explicit request or existing approved contract can settle a choice; do not ask again merely to fill in a planning record. If new evidence changes an approved choice, ask only about that change. Do not treat silence or a reviewer preference as approval, or silently drop requested work.

For non-trivial plans, keep a compact decision-coverage record in `plan-hardening-loop`: what is settled and why, routine defaults, and any open consequential choices with their affected work. Update the record from existing authorization and evidence yourself; missing formatting or a stale revision label is not a reason to restart approval. Quick questions, audits, and routine bounded fixes need no formal planning record.

For new flows or material changes to the user experience, inspect the current surface and show the lightest useful mockup together with the concrete journey before implementation. Use realistic German copy and include meaningful error/recovery or responsive variants. One explicit approval of that concrete proposal covers the design, journey, and settled decisions; reuse it after review unless a material change needs a new decision. General enthusiasm before seeing the proposal does not approve unseen behavior.

Exact, routine edits with no unresolved consequential choice, and faithful implementation of an already reviewed design, can proceed from the request or existing approval. Verify the affected result in context. A small diff is not an exemption when it changes product semantics, access, payment, or material risk. Use `prototype` only when a named interaction or logic question cannot be settled by lighter evidence, and retain the decision evidence with the plan.
