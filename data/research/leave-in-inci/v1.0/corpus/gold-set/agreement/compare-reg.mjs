...
const ref = JSON.parse(readFileSync(new URL("../reference-key/reference-key.json", import.meta.url)));
const blind = JSON.parse(readFileSync(new URL("../reference-key-v2/reference-key.json", import.meta.url)));

const bySlot = (records) => Object.fromEntries(records.map((r) => [r.slot, r]));
const R = bySlot(ref.records);
...
for (const slot of Object.keys(R).map(Number).sort((a, b) => a - b)) {
  ...
  push(slot, "g0", val(r.g0), val(b.g0_state ?? b.g0));
  const rEx = String(val(r.g0)).startsWith("excluded");
  const bEx = String(val(b.g0_state ?? b.g0)).startsWith("excluded");
  ...
  for (const d of DIMS) push(slot, `dim.${d}`, sorted(val(r.dimensions?.[d])), sorted(val(b.dimensions?.[d])));
  ...
  for (const d of DIMS) {
    const rc = conf(r.dimensions?.[d]), bc = conf(b.dimensions?.[d]);
...
writeFileSync(new URL("./regression-v1-v2.json", import.meta.url), JSON.stringify({ stats, disagreements, conf_deltas: rows.filter((r) => r.confOnly) }, null, 2));

let md = `# Agreement diff — REGRESSION ref-key v1 vs ref-key v2 (2026-09-03)\n\nCompared ${stats.compared_fields} value fields · ${stats.agreements} agree · **${stats.disagreements} disagree** (${stats.agreement_rate} agreement)\n\n| Slot | Field | Reference | Blind |\n|---|---|---|---|\n`;
...
writeFileSync(new URL("./regression-v1-v2.md", import.meta.url), md);
