...
const blind = JSON.parse(readFileSync(new URL("../blind/blind-review.json", import.meta.url)));

const bySlot = (records) => Object.fromEntries(records.map((r) => [r.slot, r]));
const R = bySlot(ref.records);
const B = bySlot(blind.records);
...
for (const slot of Object.keys(R).map(Number).sort((a, b) => a - b)) {
  ...
  push(slot, "g0", val(r.g0), val(b.g0));
...
writeFileSync(new URL("./agreement-diff.json", import.meta.url), JSON.stringify({ stats, disagreements, conf_deltas: rows.filter((r) => r.confOnly) }, null, 2));
...
writeFileSync(new URL("./agreement-diff.md", import.meta.url), md);
