...
Revision: 0.41 — independent review reconciled; provider operation and admission contracts added, 12 September 2026.
Current execution companion: [implementation-sequence.md](implementation-sequence.md). Original revision 0.36 is preserved in [archive/interview-v036.md](archive/interview-v036.md); original user acknowledgements below remain authoritative.
...
Excluded: migration of existing customer subscriptions; reactivation of the parked freemium program; refinement locks (Nick deferred them); native iOS/App Store billing; unrelated recommendation changes. Acquisition scope and returning-customer eligibility are confirmed below; provider-specific recovery differences remain under review.
...
- D05/O3 confirmed on 12 September 2026: after the PayPal restoration flow was explained, Nick accepts it, saying “Yeah it's a super weird case but I would hope that this works.” Interpret this as agreement to the described customer flow, subject to provider feasibility: “Abo fortsetzen” opens PayPal approval, preserves the original trial deadline and agreed price, and abandoned restoration leaves cancellation effective. The same renewed-approval principle is acceptable for trial interval changes where needed, with no additional trial days. This is not evidence that the provider replacement/switch mechanism works. Verification must prove original-deadline preservation, approval abandonment, single collectible agreement and recovery from interrupted handover before release; report a material incompatibility rather than changing customer behavior silently.

- D06/O1 confirmed on 12 September 2026 after the recovery tradeoff was explained: Nick chooses the second option, fresh paid checkout after failed first payment, saying “lets try to be fair here” and that it should “run 1 year from the payment date.” The complete first paid annual term and next renewal date are anchored to the verified successful payment date, not the original failed invoice/trial-end date; monthly receives a full calendar month on the same principle. Example: trial expires 20 September 2026, first successful annual payment 23 September 2026, next annual renewal 23 September 2027. Product access remains locked between expiry and successful payment. This confirms the customer-facing recovery choice, not an unverified provider mechanism; prevent old/new double collection. Later-renewal seven-day grace remains unchanged and is not automatically reanchored by this decision. Nick also reconfirms waitlist sunset. No product/provider mutation by this record.

- D02 waitlist sunset confirmed on 12 September 2026: Nick says the waitlist can be sunset and that no traffic is directed to it anymore. Retire new waitlist signup as part of launch; do not infer that historic signups or founding-price promises never existed. No deletion of existing signup records or outreach is requested. This records scope approval; the production page has not changed.
...
| D07 | Inherit existing data retention: retain account/profile/plan, lock product content when access ends, leave payment recovery and existing account-management rights available. Existing reactivation copy confirms retained profile/context. New-cohort reactivation must suppress its personal routine preview. No new deletion/export policy. | Settled through reuse; access-route verification remains |
| D08 | Reuse existing membership-management entry and cancellation interaction; add accurate trial dates and required confirmation. No additional retention offers, feedback gates, refund policy, or withdrawal-policy changes. PayPal reversal requires renewed authorization and must be included in final journey review. | Existing behavior inherited; provider-specific journey pending |
...
| D13 | Trial receives the current paid product feature set and current usage limits. Preserve existing manual/QA/field-test admission rules; never use the trial flag to enroll exempt cohorts. Refinement locks explicitly remain out of this release. | Settled through full-access and reuse decisions; parity verification remains |
...
| --- | --- | --- |
| E01 | Current code and tests: Stripe/PayPal creation, activation, events, access, cancellation, billing UI, communication and analytics | Mapped; named execution slices in implementation-sequence.md. Refresh against implementation base before edits. |
| E02 | Current entry and billing surfaces; approved German mobile offer and downstream lifecycle evidence | Offer and scanner asset approved. Downstream recovery/reapproval states and final full journey remain pending. |
| E03 | Provider schedule feasibility: seven free days → first paid year EUR 69.99 → annual EUR 99.99, cancellation parity, reminders and tax treatment | Official-doc feasibility checked; provider execution proof, tax, and required-notice coverage remain release gates. No sandbox resources. |
| E04 | Counterpart review by Claude; read-only, terminal reviewer; material findings ledger | Revision 0.40 reviewed; findings reconciled in review-resolution.md. Provider proof and final evidence remain open. |

Evidence review: pending. User-journey sign-off: pending.

## Ordered tasks

The concrete dependency-ordered tasks, file seams, interfaces and named verification are now in [implementation-sequence.md](implementation-sequence.md). The five areas below are a summary, not a substitute for those tasks:
...
Execution remains subject to the explicitly listed open choices and final reviewed journey, rather than reopening confirmed commercial decisions. Independent planning and local verification preparation are authorized by Nick's instruction to move toward implementation.
...
Findings ledger: [review-resolution.md](review-resolution.md). Claude reviewed revision 0.40 read-only; accepted as a planning artifact, not an implementation handoff. Revision 0.41 incorporates verified technical corrections, rejects reopening agreed dual-provider/identity scope, and records the remaining provider-validation boundary. Full journey sign-off is still pending.
