...
### 2026-08-09 internal-owner production rollout

- The signed-off five-stage UI remains unchanged. This rollout adds authorization only, so no new visual evidence is required.
- `PERSONAL_PLAN_APP_V1_ROLLOUT=off|internal|all` is the whole-app cohort gate. Invalid values fail closed to `off`; an absent value preserves the reviewed boolean behavior (`PERSONAL_PLAN_APP_V1_ENABLED=true` means `all`, otherwise `off`).
- `internal` uses the existing server-owned `profiles.is_admin` signal plus an optional exact `PERSONAL_PLAN_APP_V1_INTERNAL_EMAILS` allowlist resolved from the confirmed Supabase Auth user. The production audit found exactly one admin account, Nick's. Browser metadata, quiz answers and checkout metadata never participate in access.
- The owner-scoped journey loader rejects excluded users as `legacy` before entitlement, prepared-artifact, plan, product, Routine, or application reads. Every Personal Plan API already depends on this loader after authenticated `auth.getUser()` resolution.
- `/welcome` and `/plan-bereit` use the same server-side cohort decision before redirecting or linking to `/plan-start`; excluded purchasers retain the established onboarding-to-Routine journey.
- A fresh-account production test uses a dedicated Gmail plus-address configured only in the server-side Vercel environment. Checkout-created accounts are email-confirmed by the existing Stripe and PayPal activation paths before rollout eligibility is evaluated; the address is not committed to the repository.
- Internal production testing requires all four stage code flags enabled, `PERSONAL_PLAN_APP_V1_ROLLOUT=internal`, `PERSONAL_PLAN_APP_V1_INTERNAL_EMAILS` set to the test address, and `PERSONAL_PLAN_STAGE5_ROLLOUT=internal`. Moving to customers is a later explicit environment-only decision to set both rollout variables to `all`; the global app kill switch remains independently reversible.
