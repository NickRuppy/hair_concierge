...
### Consolidated Orchestration Snapshot

This is the current master ledger for the product-intake stack. It was reconciled on 2026-06-29
from the active integrated worktree and the more recent canonical-identity ledger updates.

Source-of-truth rules:

- Use `.worktrees/product-intake-full-flow-smoke` as the active integration worktree for the
  current full-flow smoke, clarification-card, selected-product, and submit-polish work.
- Use `.worktrees/product-identity-canonical-correction` only as historical evidence for the
  canonical identity correction and historical free-text replay work. Its relevant ledger updates
  are folded into this master ledger; do not continue splitting product-intake decisions across
  both worktrees.
- Do not use the root checkout for this stack. It is behind `origin/main` and has unrelated dirty
  files.
- Treat old phase-specific worktrees as PR/source branches, not active planning locations, unless
  a concrete PR fix must be made there.

Current worktree status:

| Worktree | Branch / role | Current state | Work still needed |
| --- | --- | --- | --- |
| `.worktrees/product-intake-full-flow-smoke` | `codex/product-intake-full-flow-smoke`; integrated top-stack smoke worktree | Active. Contains the merged Phase 0-5 stack plus local uncommitted/untracked integration work for clarification cards, selected-product architecture cleanup, chat submit polish, research-ops workflow, and the 2026-06-28 product-assessment/context/lookup rework. Local full-flow smoke and research-ops dry-runs were run on 2026-06-26. On 2026-06-27 the end-of-package image upload/verification gate and the OLAPLEX duplicate-prevention lookup correction were added locally and focused tests passed. On 2026-06-28 the product-assessment implementation, Claude fixups, `requesting-code-review` fixups, fresh-context subagent guardrail fixes, bounded pending-submission refresh, and post-quota live-smoke fixes were applied locally. On 2026-06-29 the integrated local smoke phase was accepted as complete after the remaining UX/logic fixes: non-duplicative intake copy, submitted-card collapse, selected-product clarification continuity, selected-card UI lock, category-wide product-assessment context, repeat-intake suppression, and research-ops review/app/image-package dry runs. The final patch set also tightened weak exact-lookup matches, retried missing product-review notifications, required verified category specs before owned non-recommended products become assessable, preserved resolved assessment targets when category engines return no displayable products, and wired the new resolved-selection test into `test:agent`. The latest post-review patch additionally makes product-review notifications deterministic/idempotent by submission/status, treats post-insert conversation bumps as best-effort, preserves structured SSE error messages, prefers explicit product-type words over oil ingredient wording during category-less lookup, and passes lookup-sourced exact product hints into product assessment. Targeted tests, `npm run typecheck`, `git diff --check`, historical replay, and `npx tsx tmp/live-product-assessment-smoke.ts all` pass locally; the latest patch set is additionally covered by focused lookup, notification, chat UI, product-selection, production-pipeline, select-products, resolved-selection, and typecheck runs. The final post-patch broad check `npm run test:agent` passed locally on 2026-06-29 with 901/901 tests after the verified-spec readiness helper accepted both DB-shaped and normalized catalog-shaped category fields. | Smoke phase complete. Remaining gates are not smoke work: decide whether to create the Codex daily automation, decide whether to run one approved apply/confirm test submission, check target Supabase readiness before any real approval write, and run the final shipping gate. Stage/commit/push/PR only after explicit approval. Preserve unrelated `supabase/.temp/*` and exclude `data/product-additions/2026-06-27-user-submitted.json` unless/until the product intake flow is clean enough to deliberately package it. |
| `.worktrees/product-identity-canonical-correction` | `codex/product-identity-canonical-correction`; PR #182 / canonical identity evidence | Contains canonical identity correction plan updates and the historical replay v2 script/test/results. Relevant ledger updates are imported here. | No new product-intake implementation should continue there unless PR #182 needs a direct fix. Decide later whether to preserve the replay script/test in the final stack or archive it as calibration tooling. |
| `.worktrees/product-intake-intelligence` | `codex/product-intake-intelligence`; PR #172 | Phase 0 branch. Refreshed with `origin/main` on 2026-06-24 and reported `CLEAN` in the previous ledger. | No local feature work expected. Keep as lower stacked PR branch; merge/refresh only during stack shipping. |
| `.worktrees/product-intake-phase-1` | `codex/product-intake-phase-1`; PR #173 | Clean against its remote in the latest local status check. | No local feature work expected; stack shipping only. |
| `.worktrees/product-intake-phase-2` | `codex/product-intake-phase-2`; PR #175 | Clean against its remote in the latest local status check. | No local feature work expected; stack shipping only. |
| `.worktrees/product-intake-phase-3` | `codex/product-intake-phase-3`; PR #177 | Code branch clean except untracked Claude/review note files. | No product work expected unless PR #177 gets review comments; optionally archive/remove untracked review notes during cleanup. |
| `.worktrees/product-intake-phase-4a-review-core` | `codex/product-intake-phase-4a-review-core`; PR #179 | Clean against its remote in the latest local status check. | No local feature work expected; stack shipping only. |
| `.worktrees/product-intake-phase-4b-ops` | `codex/product-intake-phase-4b-ops`; PR #180 | Clean against its remote in the latest local status check. | No local feature work expected; stack shipping only. |
| `.worktrees/product-intake-phase-5` | `codex/product-intake-phase-5`; PR #181 | Local branch is ahead of remote by 2 in the latest status check. Related updates appear to be represented in later identity/full-flow work. | Reconcile before shipping PR #181: either push/confirm those two commits are intentionally part of the stack, or verify they are superseded by PR #182/top-stack work. |

Current plan-file status:

| Plan file | Status | Work still needed |
| --- | --- | --- |
| `plans/2026-06-10-product-intake-intelligence.md` | Master ledger and long-form source of truth. This top section is consolidated as of 2026-06-29. | Keep this ledger updated after the final review gate, shipping decisions, Supabase readiness checks, and any migration checks. Detailed legacy checkboxes below may still lag the PR state. |
| `plans/2026-06-22-agent-native-product-lookup.md` | Implemented. Covers model-owned `specific_product_candidate`, typed `lookup_product_candidate`, validator backstop, and historical replay. | No implementation work. Keep as evidence/spec for PR #182 behavior. |
| `plans/2026-06-25-product-lookup-clarification-card.md` | Implemented in the integrated smoke worktree; latest review findings patched. Local full-flow smoke covered unknown-product intake, ambiguous-product clarification, candidate selection, no-card broad prompts, and the post-selection locked-card UI state. | Final review/shipping gate only. |
| `plans/2026-06-25-product-selection-architecture-cleanup.md` | Implementation complete in the integrated smoke worktree. Local full-flow smoke covered candidate selection and active-product follow-up; the selected-card UI lock now derives from persisted `product_lookup_selection` metadata. | Final review/shipping gate only. |
| `plans/2026-06-25-product-intake-chat-submit-polish.md` | Implemented in the integrated smoke worktree; focused tests, typecheck, and browser smoke passed. Latest superpowers review found no blocking issues. | Final review/shipping gate only. |
| `plans/2026-06-26-product-intake-research-ops.md` | Implemented in the integrated smoke worktree through read-only prep, package contract, review-lane default, dry-run approval wrapper, review app, local package approval artifact, image finalization, and Supabase Storage upload/verify before DB approval. Fresh review findings were patched: incomplete package rejection, metadata id mismatch rejection, ready-for-review write gate, signed image-review metadata, final image decision validation, renderable local image candidates, compact property review, guarded final local package approval, image upload checksum/download verification, and the OLAPLEX No.7 duplicate-prevention lookup correction. Focused tests, typecheck, package generation, dry-run approval wrapper, and pending backlog report passed on 2026-06-26; focused image-upload and lookup tests passed on 2026-06-27. The operator runbook is documented in `docs/product-intake-research-ops.md`; local `ops/product-intake-research/**` packages are gitignored. | No Supabase approval write, migration command, or Codex automation was run on 2026-06-27. Next decisions: check target Supabase migration/readiness state, run a final review gate for the review app/workflow, create the daily read/prep automation after explicit approval, optionally run one explicit apply/confirm test approval, then include this in the final shipping gate. |
| `plans/2026-06-28-product-assessment-context-and-lookup.md` | Implemented locally in the integrated smoke worktree. Covers first-class `product_assessment`, agent-directed candidate retrieval, max-3 active product contexts, pending submission context, and assessment-vs-recommendation boundaries. Claude review, `requesting-code-review`, and fresh-context subagent findings were patched; targeted tests, typecheck, diff hygiene, historical replay, and live smoke pass after the final visible-resolved-product-name and category-less known-brand fallback patches. The bounded pending-submission refresh and repeat-intake suppression patch is implemented and covered by deterministic production-pipeline tests. The final review fixups for verified-spec ownership gating and resolved-target assessment preservation are implemented and covered by focused route, pipeline, and select-products tests. | Include this scope in the final integrated simulated-user/review gate. |
| `plans/2026-06-18-product-identity-canonical-correction.md` | Canonical identity correction plan in the identity worktree. Phase A correction and historical replay are implemented as evidence. | Guarded production apply remains a separate explicit operation; do not run it as part of the current merge unless explicitly approved. |

Recommended next steps:

1. Continue in `.worktrees/product-intake-full-flow-smoke`.
2. Treat the integrated local smoke phase as complete; do not reopen smoke unless a new blocking behavior appears.
3. Treat the latest integrated review findings as patched. The 2026-06-29 final patch tests, `npm run typecheck`, `git diff --check`, and broad `npm run test:agent` have passed locally; no staging/commit/push/PR has been done for them yet.
4. Check the target Supabase migration/readiness state before any real approval write.
5. Decide whether to create the proposed daily Codex research-prep automation.
6. Decide whether to run one explicit `approve-package --apply --confirm` test submission after
   filling/reviewing a real package payload.
7. Keep `data/product-additions/2026-06-27-user-submitted.json` out of the final commit unless/until the product intake flow is clean and the product package is deliberately approved for inclusion.
8. Reconcile `.worktrees/product-intake-phase-5` being ahead by 2 before shipping PR #181.
9. If desired, run one last post-patch autoreview before shipping; otherwise move to the final shipping gate.
10. Only after explicit approval, stage/commit/push the integrated work and decide whether it becomes
   a new top stacked PR or updates the current top-stack branch.
11. After shipping approval, merge the stack in order: #172 -> #173 -> #175 -> #177 -> #179 ->
   #180 -> #181 -> #182 -> the integrated full-flow/smoke PR if it remains separate.

### Integrated Local Smoke - 2026-06-26

Smoke worktree: `.worktrees/product-intake-full-flow-smoke`

Artifacts:
`/Users/nick/AI_work/hair_conscierge/.worktrees/product-intake-full-flow-smoke/tmp/full-flow-smoke-2026-06-26`

Fresh checks run:

- `npx tsx --test tests/agent-v2-product-selection.spec.ts tests/agent-v2-product-lookup-clarification.spec.ts tests/chat-product-mentions.test.tsx tests/agent-v2-production-chat-pipeline.spec.ts tests/resolved-product-selection.test.ts tests/agent-v2-resolved-product-selection-adapter.spec.ts` -> 85/85 passing.
- `npm run typecheck` -> passing.

Browser smoke evidence:

- Onboarding photo path submitted a supported-category product with front image, category, and frequency, then advanced to the next onboarding section.
- Onboarding manual path submitted through `/api/product-intake/onboarding` successfully. The direct URL smoke showed some onboarding-state/category carryover, so treat this as API/flow evidence rather than a polished deep-link category assertion.
- Chat unknown-product path rendered the intake card from structured lookup metadata.
- Chat submit collapsed the editable card into the pending-review confirmation.
- Chat "none/add product" from a clarification card revealed the intake card.
- Chat candidate selection produced a follow-up assistant answer with the selected product context.
- A non-product Frizz question in a fresh chat did not trigger product lookup/intake and answered normally.

Smoke disposition:

- The integrated local smoke phase is complete as of 2026-06-29. The earlier candidate-clarification copy, selected-product continuity, repeat-intake, submitted-card, and selected-card lock findings were patched and covered by focused tests plus local chat smoke. Remaining work is final review/shipping and explicit production/Supabase operations, not more smoke-test discovery.

| Phase                                               | Status                                       | Branch / PR                                                                                                                | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------- | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase 0: Product Identity Normalization             | Implemented, pushed, draft PR open           | `codex/product-intake-intelligence`, PR #172, base `main`                                                                  | Open draft PR, mergeable, visible GitHub/Vercel checks green as of 2026-06-13. The Phase 0 schema migration is applied on production, and the guarded product identity backfill was applied on 2026-06-18 with `products:identity:apply -- --apply --confirm-project=pqdkhefxsxkyeqelqegq`. Post-backfill verification showed 239 total products, 0 missing `brand_id`, 0 missing `category_key`, 75 brands, 99 brand aliases, 23 product lines, and product-intake readiness passing. Keep legacy `products.brand` and `products.category` until Phase 7 after a separate production release.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Phase 5: Assistant And Recommendation Integration   | Implemented, reviewed, pushed, draft PRs open; migration gate complete | `codex/product-intake-phase-5`, PR #181, base `codex/product-intake-phase-4b-ops` / PR #180; `codex/product-identity-canonical-correction`, PR #182, base `codex/product-intake-phase-5` | PR #181 implements explicit product eligibility modes, AgentV2 product lookup validation, owned verified product context, admin product filters, and a product visibility policy migration (`20260618120000_product_visibility_policy_lifecycle.sql`). PR #182 completes the post-smoke architecture correction: model-owned `specific_product_candidate`, agent-native `lookup_product_candidate` tool use, structured lookup-status card gating, validator/tool-trace enforcement, lowercase named-product recognition, focused regressions, manual browser smoke, and read-only historical `user_product_usage` replay. PR #182 also includes the reviewed Phase A canonical identity correction plan/script/data updates; the guarded production apply remains a separate explicit operation. Final verification already run for PR #182: focused AgentV2 tests, product-intake replay tests, `npm run typecheck`, strict historical replay, reviewed replay-v2 calibration, Claude review, Clawpatch/autoreview, and 2026-06-23 browser smoke for unknown-product card, ambiguous-product clarification/no-card, and broad brand/category no-card. PR #181 and PR #182 were draft PRs with clean merge state and passing visible Vercel checks in the previous ledger. The PR #181 Supabase migration gate was run on 2026-06-23 with `npx supabase db push --dry-run` and `npx supabase db push --yes`; both reported `Remote database is up to date`, so no migration was applied. Remaining before merge: reconcile the active full-flow smoke worktree and stack merge planning. No migration files changed in PR #182. |
| Phase 6: Verification And Shipping Gates            | Ongoing per phase                            | PR stack #172 -> #173 -> #175 -> #177 -> #179 -> #180 -> #181 -> #182 -> integrated full-flow smoke branch if kept separate | Final merge/deploy requires live PR checks, review gates, explicit Supabase migration-state decisions, and stack refresh/merge planning. Stack review in the previous ledger: all PRs were still draft; PR #172 was refreshed on 2026-06-24 by merging `origin/main` into `codex/product-intake-intelligence`, rerun checks passed, and GitHub reported PR #172 `CLEAN`; PR #173, #175, #177, #179, #180, #181, and #182 were also reported `CLEAN`. Immediate next gate: complete one top-stack local full-flow smoke in `.worktrees/product-intake-full-flow-smoke`, reconcile the phase-5 branch ahead-by-2 state, then decide whether to mark/merge the stack or open/update a final integrated smoke PR. |
...
### Historical Free-Text Replay Interpretation

The historical `user_product_usage.product_name` replay is complete and should not block merge.
It was useful as calibration, but it is not a clean product-matcher quality score because the old
onboarding flow stored noisy one-field free text instead of the new split fields and photos.

Final calibration summary:

- Old rows were mostly incomplete: nearly all lacked structured brand and product id.
- Strict lookup was intentionally conservative and therefore found almost no exact matches.
- Replay v2 split likely brand/line/name, detected multi-product rows, and produced review buckets
  instead of pretending old free text was reliable enough for auto-linking.
- Nick reviewed a representative subset and confirmed the pattern: many rows are discardable,
  unsupported, too generic, or need research/splitting.
- Decision: mark historical replay testing complete. Do not require manual review of the remaining
  old rows before merge. Future product-intake data will be more valuable because it collects
  category, frequency, brand/name and/or photos at submission time.

Known replay artifacts from the identity worktree:

- Script: `scripts/product-intake/replay-user-product-usage-review.ts`
- Test: `tests/product-intake-replay-user-product-usage-review.spec.ts`
- Reviewed CSV: `tmp/product-usage-review-replay-2026-06-23T18-45-23-789Z.csv`
- Reviewed decision sync: `tmp/product-usage-review-decisions-live.json`
...
- Approval requires complete source-backed metadata in one conclusive research pass: commercial link, final image decision, price, currency, link status, checked timestamps, source evidence, category-specific specs, and manual review. A raw retailer/brand image URL is source evidence only; a DB-ready `products.image_url` should be a reviewed, normalized Chaarlie-hosted asset in `product-images`, unless the reviewer explicitly approves the product without an image for now.
...
### Task 5.2A: Product Resolution Invariant Patch

**Why this exists:** local smoke on 2026-06-19 proved that the chat card and submit APIs work when `rag_context.product_intake_offer` exists, but a natural own-product suitability ask can still be answered through category-level AgentV2 advice without calling `lookup_product_candidate`. That is architecturally wrong: product identity resolution must be a pre-answer contract for concrete own-product use/opinion/add turns, not a best-effort prompt suggestion.

**Current status:** Implemented in PR #182 through the focused plan `plans/2026-06-22-agent-native-product-lookup.md`. The implementation deliberately moved away from broad deterministic phrase matching: the model owns `specific_product_candidate`, `lookup_product_candidate` is the typed lookup boundary, and card rendering is downstream of current-turn structured lookup metadata. Focused checks, manual browser smoke, strict historical replay, and reviewed replay-v2 calibration are complete. The current remaining gate is the integrated top-stack full-flow smoke in `.worktrees/product-intake-full-flow-smoke`.

**Files:**

- Modify: `src/lib/agent-v2/named-product-context.ts`
...
- [x] **Step 1: Add behavior-level failing tests for own-product resolution**

Add tests that encode the invariant, not literal one-off smoke strings:

- own-product suitability/use question with a concrete supported-category product requires `lookup_product_candidate` before any final answer;
- unknown supported-category lookup result yields `product_intake_offer`;
- broad category recommendation without a concrete product candidate does not require lookup and does not render intake;
- ambiguous product identity asks clarification and does not render intake;
- unsupported product category returns the friendly unsupported-category path and does not render intake.

The test data may use generated or fake product names, but assertions must target behavior: required lookup, lookup status, absence/presence of `product_intake_offer`, and no category-only suitability answer for unresolved named products.

- [x] **Step 2: Strengthen named-product context as a product-resolution signal**

Extend `src/lib/agent-v2/named-product-context.ts` only as needed so common user phrasing is recognized as a concrete product candidate:

- "Ich benutze/nutze/verwende ... passt das zu mir?"
- "Was hältst du von meinem ...?"
- "Ich möchte ... in meine Routine aufnehmen/hinzufügen."

Keep the detector conservative:

- supported category/use must be inferable from explicit wording, product name, or active context;
- broad asks like "Welcher Leave-in passt?" or "Welche Pantene Produkte empfiehlst du?" remain non-intake;
- no deterministic full German response templates are introduced.

- [x] **Step 3: Make lookup a runtime pre-answer requirement, not just prompt guidance**

In `src/lib/agent-v2/runtime/responses-agent.ts`, derive a product-resolution requirement from `namedProductContext`, `productIntakeEnabled`, and the user intent class before accepting a terminal answer.

The runtime should require `lookup_product_candidate` for concrete own-product suitability/use/add turns even if the model tries to self-classify the answer as `general_advice` or route through `select_products`.

Do not make this a broad product-search rewrite:

- recommendation/category asks stay on `select_products`;
- product lookup remains focused on concrete named product candidates;
- the model still owns natural German wording and clarification phrasing.

- [x] **Step 4: Keep the validator as a backstop**

Update `src/lib/agent-v2/validation/final-answer-validator.ts` so the validator blocks terminal answers when product resolution is required and `lookup_product_candidate` was not called.

The block must not depend only on the model's self-declared `request_interpretation.product_request_kind`; it should also respect the runtime's named-product/product-resolution requirement.

Validator behavior:

- missing lookup on a concrete own-product suitability/use/add turn is blocking;
- unresolved lookup (`not_found`, `ambiguous`, `insufficient_identity`, `unsupported_category`) blocks exact product-specific assessment and product/property claims;
- category-level caveats are allowed only when phrased as non-evaluative context after deferring the exact product.

- [x] **Step 5: Let lookup status drive the final chat outcome**

Keep the existing result model as the only source of chat intake offers:

- `found_exact`: answer from verified product/catalog properties; no intake card;
- `not_found`: natural defer plus `rag_context.product_intake_offer`;
- `ambiguous`: ask which variant; no intake card;
- `insufficient_identity`: ask for missing product/category detail; no intake card;
- `unsupported_category`: friendly unsupported-category reply; no intake card.

Do not reintroduce raw-message regex card rendering, deterministic response templates, or a `productsToSend.length === 0` card gate.

- [x] **Step 6: Re-run focused checks and the exact smoke path**

Focused checks, historical replay, reviewed replay-v2 calibration, and PR #182 manual browser smoke passed:

- unknown concrete product -> natural deferral and product-intake card;
- ambiguous product -> clarification and no card;
- broad category/brand ask -> no intake card.

Additional integrated smoke remains for the later `.worktrees/product-intake-full-flow-smoke`
changes: onboarding intake, chat intake submission, selected-product clarification, collapsed
submitted state, and a non-product chat sanity path.

Run focused tests first:

```bash
npm run test:node -- tests/agent-v2-named-product-context.spec.ts tests/agent-v2-final-answer-validator.spec.ts tests/agent-v2-responses-runtime.spec.ts tests/agent-v2-production-chat-pipeline.spec.ts
```

Then run the product-intake/chat verification that proves the promised end state:

- onboarding manual path still creates/updates pending review;
- onboarding photo path still creates/updates pending review with front/barcode images;
- synthetic chat card manual/photo submits still create pending review rows;
- natural chat own-product suitability ask for an unknown supported-category product now defers and stores/sends `rag_context.product_intake_offer`;
- broad category recommendation ask does not show the intake card.

If the natural chat path still answers category advice without a lookup, stop and inspect the AgentV2 tool-call trace before trying another patch.
...
- [x] Onboarding: add product by photo path with front photo, category, frequency, no barcode.
...
- [x] Chat: ask about unknown product and verify assistant defers with product-intake card.
- [x] PR #182 manual smoke: ask about a lowercase unknown concrete supported-category product and verify `lookup_product_candidate` runs, the assistant defers product-specific judgment, and the intake card appears.
- [x] PR #182 manual smoke: ask an ambiguous named-product question and verify the assistant asks for clarification and no intake card appears.
- [x] PR #182 manual smoke: ask a broad brand/category recommendation question and verify no intake card appears.
- [ ] Chat: submit product intake by photo with a large phone-style image and verify compression plus upload success.
- [x] Chat: submit manual text-only product and verify pending review state.
...
- [x] Phase 5 migration gate: before merging PR #181, check `20260618120000_product_visibility_policy_lifecycle.sql` against Supabase project `pqdkhefxsxkyeqelqegq` and explicitly record whether it is already applied, applied during shipping, or still unapplied. Completed on 2026-06-23; Supabase reported the remote database already up to date and no migration was applied.
...
8. Phase 6 verification and shipping. PR #182 smoke fixups, reviewed replay-v2 calibration, live checks, and PR #181 migration-state gate are complete in the previous ledger. Current concrete step: run the final integrated simulated-user/review gate in `.worktrees/product-intake-full-flow-smoke`, then choose whether to ship it as a final stacked PR or fold it into the current top branch.
...
Recommended next steps from the integrated smoke worktree:

1. Continue in `.worktrees/product-intake-full-flow-smoke`.
2. Run the final integrated simulated-user/review gate for product assessment, clarification cards, intake submission/pending behavior, and review-ops surfaces.
3. Reconcile `.worktrees/product-intake-phase-5` being ahead by 2 before shipping PR #181.
4. Check target Supabase migration/readiness state before any real approval write.
5. Only after explicit approval, stage/commit/push and decide whether this becomes a new top stacked PR or updates the current top branch.
6. Do not run the guarded canonical identity production apply or clean temporary uploads as part of this merge unless explicitly approved.
