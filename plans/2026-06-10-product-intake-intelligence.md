...
| Phase 5: Assistant And Recommendation Integration   | Implemented, reviewed, pushed, draft PR open; post-smoke patch required before merge | `codex/product-intake-phase-5`, PR #181, base `codex/product-intake-phase-4b-ops` / PR #180                                | Implements explicit product eligibility modes, AgentV2 product lookup validation, owned verified product context, admin product filters, and a product visibility policy migration (`20260618120000_product_visibility_policy_lifecycle.sql`). Initial spec-review blockers were patched: user-visible lookup is active-lifecycle recommended-only while intake dedupe remains active-only, named-product claims require lookup and unresolved lookup blocks product-specific prose, and owned non-recommended products no longer expose a user-facing not-recommended caveat. Code-review findings were patched so explicit owned IDs use owned-assessment eligibility while normal global candidates remain recommended-only, including a shared guard that blocks owned non-recommended products without category-specific specs/eligibility rows across the supported rerankers and matcher preservation so explicit owned IDs are not truncated before reranking. Follow-up review fixups patched the production AgentV2 routine identity projection (`product_id`, `product_submission_id`, `match_status`), added narrow RLS for users to read active products matched into their own routine, and made lookup-required repair call `lookup_product_candidate` before normal product grounding. Local smoke on 2026-06-19 proved onboarding text/photo intake and synthetic chat-card text/photo submits, but natural chat still allowed a precise own-product suitability ask to escape into category-level leave-in advice with `rag_context.product_intake_offer = null`. Before merge, complete the focused AgentV2 architecture correction in `plans/2026-06-22-agent-native-product-lookup.md`: model-native `lookup_product_candidate` tool choice, one required `request_interpretation.specific_product_candidate` boolean, validator/tool-trace enforcement, structured lookup-status intake behavior, retiring deterministic lookup enforcement, and read-only historical `user_product_usage` replay. Clawpatch dirty-diff review patched and revalidated four relevant findings; final read-only subagent review found no blockers before this new smoke finding. Focused Phase 5 tests passed (330 passing), broad node suite excluding the known PayPal checkout test passed (810 passing), diff hygiene and Prettier passed, and typecheck is blocked only by the pre-existing PayPal module/type issue. **Before merging PR #181, the Phase 5 migration remains unapplied from this worktree and must go through the Supabase migration-state gate for project `pqdkhefxsxkyeqelqegq`.** |
| Phase 6: Verification And Shipping Gates            | Ongoing per phase                            | PR stack #172 -> #173 -> #175 -> #177 -> #179 -> #180 -> #181                                                              | Final merge/deploy requires live PR checks, review gates, explicit Supabase migration-state decisions, and a stack refresh because PR #172 is currently behind `main` as of 2026-06-18.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- [ ] **Step 1: Add behavior-level failing tests for own-product resolution**
...
- [ ] **Step 2: Strengthen named-product context as a product-resolution signal**
...
- [ ] **Step 3: Make lookup a runtime pre-answer requirement, not just prompt guidance**
...
- [ ] **Step 4: Keep the validator as a backstop**
...
- [ ] **Step 5: Let lookup status drive the final chat outcome**
