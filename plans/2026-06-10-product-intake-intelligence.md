...
| Phase 5: Assistant And Recommendation Integration   | Implemented, reviewed, pushed, draft PRs open; migration gate remains before merge | `codex/product-intake-phase-5`, PR #181, base `codex/product-intake-phase-4b-ops` / PR #180; `codex/product-identity-canonical-correction`, PR #182, base `codex/product-intake-phase-5` | PR #181 implements explicit product eligibility modes, AgentV2 product lookup validation, owned verified product context, admin product filters, and a product visibility policy migration (`20260618120000_product_visibility_policy_lifecycle.sql`). PR #182 completes the post-smoke architecture correction: model-owned `specific_product_candidate`, agent-native `lookup_product_candidate` tool use, structured lookup-status card gating, validator/tool-trace enforcement, lowercase named-product recognition, focused regressions, manual browser smoke, and read-only historical `user_product_usage` replay. PR #182 also includes the reviewed Phase A canonical identity correction plan/script/data updates; the guarded production apply remains a separate explicit operation. Final verification already run for PR #182: focused AgentV2 tests, product-intake replay test, `npm run typecheck`, historical replay (`found_exact=0`, `ambiguous=0`, `not_found=3`, `insufficient_identity=378`, `unsupported_category=17`, `skipped_missing_product_name=31`), Claude review, Clawpatch/autoreview, and 2026-06-23 browser smoke for unknown-product card, ambiguous-product clarification/no-card, and broad brand/category no-card. Remaining before merge: live PR check refresh and the Supabase migration-state gate for PR #181's visibility policy migration. No migration files changed in PR #182. |
| Phase 6: Verification And Shipping Gates            | Ongoing per phase                            | PR stack #172 -> #173 -> #175 -> #177 -> #179 -> #180 -> #181 -> #182                                                      | Final merge/deploy requires live PR checks, review gates, explicit Supabase migration-state decisions, and a stack refresh because the lower stack may be behind `main`. The immediate next gates are PR #182 live check refresh, Supabase migration-state confirmation for PR #181, then PR-stack refresh/merge planning.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- [ ] **Step 6: Re-run focused checks and the exact smoke path**

Focused checks and historical replay passed in PR #182. Manual browser smoke remains open:
...
- [ ] Phase 5 migration gate: before merging PR #181, check `20260618120000_product_visibility_policy_lifecycle.sql` against Supabase project `pqdkhefxsxkyeqelqegq` and explicitly record whether it is already applied, applied during shipping, or still unapplied.
