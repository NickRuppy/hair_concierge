...
const ANSWERS={};const nick=loader(NICK);
const first=nick('src/lib/quiz/questions.ts').quizQuestions[0];
...
const intro='<aside class="scanner-intro" aria-labelledby="intro-title"><strong id="intro-title">Damit der Scanner zu deinem Haar passt.</strong><p>Der Scanner gleicht Produkte mit deinem Haarprofil ab. Dafür brauchen wir ein paar Angaben zu deinen Haaren.</p><span>10 kurze Fragen · ca. 2 Minuten</span></aside>';
const html='<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Scanner · Direkt ins Quiz</title>'+fonts+'<link rel="stylesheet" href="quiz-entry-render.css"><link rel="stylesheet" href="frame.css"><link rel="stylesheet" href="quiz-entry.css"></head><body><main class="quiz-entry"><div class="entry-brand">chaarlie <span>Dein Produkt-Scanner</span></div>'+intro+'<section id="question" aria-label="Erste Quizfrage">'+question+'</section><p class="answer-hint">Tippe auf eine Antwort, um weiterzumachen.</p></main><aside id="entry-notice" role="status" hidden></aside><script src="quiz-entry.js"></script></body></html>';
fs.writeFileSync(path.join(out,'quiz-entry.html'),html);
...
(async()=>{const postcss=req('postcss'),tw=req('@tailwindcss/postcss');let css=source(NICK,'src/app/globals.css').toString().replace('@import "tailwindcss";',`@import "${path.join(primary,'node_modules/tailwindcss/index.css')}" source(none);\n@source "${out}";`);const built=await postcss([tw({base:out})]).process(css,{from:path.join(primary,'src/app/globals.css')});fs.writeFileSync(path.join(out,'quiz-entry-render.css'),built.css);fs.writeFileSync(path.join(out,'quiz-entry-source.json'),JSON.stringify({source:NICK,questionStep:first.step,questionNumber:first.questionNumber,assets:[...assets],adapters:['empty quiz state','inactive navigation','native image tags','local info-tip button','first-question back button hidden'],note:'Current first QuizQuestion component plus proposed short ad-entry explanation. Static preview only.'},null,2));console.log('Rendered first question from '+NICK+', assets '+assets.size)})().catch(e=>{console.error(e);process.exitCode=1});
