...
**T3 — Protocol content-template set.**
...
**T4b — Image finalization (R5).**
Run every validated pilot product through the existing finalize-image pipeline (bg removal/deshadow → `product-images` bucket asset with `product_image_assets` provenance → thumbnail), reusing `scripts/product-intake/` finalize-image tooling and the established packshot/deshadow recipes. Track per-product time cost for the retro. Completion: every product headed for apply has a finalized own-bucket `image_url`; preflight (T5) rejects any product without one.
