...
- Use `ON UPDATE RESTRICT ON DELETE CASCADE` for product/category references. Use deferred `ON DELETE NO ACTION` for contextual fact references to normalized eligibility so deleting a broad eligibility row cannot silently erase richer curated facts, while whole-product deletion and reviewed bundle replacement can settle transactionally. A guarded category repair must first make the product non-recommendable, delete the old category bundle, update the parent category, and insert/validate the new bundle in one transaction; it may not rely on cascading a category change through semantically different facts.
...
Add `product_thickness_eligibility` and `product_concern_eligibility`, backfilled deterministically from current arrays and verified category tuple rows. Reconfirm the live origin domain and null/category counts at execution time, backfill a known `category_key` for every `products` row, validate the existing two-value origin check, and add a `NOT VALID` global category non-null check for later validation. Add unique/composite keys, constant category columns, checks, indexes, and `NOT VALID` composite foreign keys. Product/category references use `ON UPDATE RESTRICT ON DELETE CASCADE`; contextual facts use deferred `ON DELETE NO ACTION` when referencing normalized eligibility. Do not delete or rewrite legacy sources in this task.
