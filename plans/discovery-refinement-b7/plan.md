# Discovery toolkit — batch 7 refinement plan (Rev. 3)

Status: **Rev. 1 — locked by Nick 2026-09-25 („lock this in … build it")**, D1/D2 ruled 2026-09-25. Rev. 2 = Codex plan review round 1 folded in; Rev. 3 = Codex confirm pass (3 unresolved + 1 new P1) folded in, see §6.
...
- `resolvePrimaryQuizConcern` (`need-lane.ts:115`) is **retired as the source of truth**: new `resolveStatedPrimaryConcern(rawAnswers)` returns `primary_concern` if valid, else the single concern if exactly one, else `null` — computed from the RAW canonical answers (full `DIAGNOSTIC_CONCERNS` vocabulary), not from `projectQuizAnswersForLegacyConsumers` (`normalization.ts:~334`), which drops `low_shine`, `lost_shape`, `low_volume_or_weighed_down` (and maps `hair_loss_or_thinning` away) (Codex confirm-pass P1). The inferred weight ranking is deleted (not kept as fallback — ruling F1.2).
- Two consumers, two vocabularies: (a) **copy** — `result-narrative.ts` (reads the reduced projection at `~935`) receives the stated concern as an explicit extra input and gets `CONCERN_COPY` entries for the four non-legacy codes (`hair_loss_or_thinning` uses the existing medical-boundary tone, no product promise); (b) **need lane** (`resolveQuizNeed`) uses the stated concern only when it maps into the legacy `QuizConcern` vocabulary (incl. aliases `dry_lengths→dryness`, `frizz_flyaways→frizz`), otherwise lane resolution runs with `primaryConcern = null` (existing null branches). Tests for all four non-legacy codes (copy present, lane falls back, no crash) plus aliases.
...
- D2: `product_type` CHECK + intake zod vocab (`src/lib/discovery/intake.ts:~306`) gain `styling`; a `styling` item has `category IS NULL` AND `product_type='styling'` (new CHECK: styling ⇒ category null, usage_role null). It is NOT „Kategorie offen": the finalize `category_open` block, research auto-enqueue (no `product_submissions` row for styling), verdicts, binding and routine all skip `product_type='styling'`; the cockpit lists it under „Styling (nicht bewertet)". `styling` stays out of `SUPPORTED_PRODUCT_CATEGORY_KEYS` (the 10 categories are untouched). Projections that must keep the marker (Codex confirm-pass P1): the participant row projection in `src/lib/discovery/intake.ts:~595` (currently nulls unsupported product types — add a discovery-local `DiscoveryProductType = PersonalPlanCategory | "styling"`), the cockpit row projection in `src/lib/discovery/cockpit.ts`, and `refined-routine.ts:~212` which must branch on `productType === "styling"` BEFORE the null-category → `category_unknown` mapping, producing unassigned reason `styling_not_evaluated` (new intakes only, so legacy hashes are unaffected). Tests: styling item never yields `category_unknown`, never blocks finalize, appears in the cockpit list.
...
- **Idealroutine uses the answers** (ruling F3.3). Build a `PlanRoutineContext` directly from intake data (NOT via `buildPlanRoutineContextFromCompletedRefinement`, whose completeness contract requires towel/night answers we deliberately don't collect): `shampooFrequency` from the most frequent shampoo item, `heatToolUse` from `projectStage2HeatEvents(heat_styling)`, `currentProductLoad` from item categories/oil roles; everything else stays `unknown`.
  Concrete path (Codex P1-1, revised in Rev. 3): the single read model `loadDiscoveryCockpitModel` (`src/lib/discovery/cockpit.ts`, shared by the cockpit page, the PDF page and the admin decisions/finalize routes via `src/app/api/admin/beratung/shared.ts`) loads intake items + `heat_styling` BEFORE the ideal routine, builds the override with the pure `buildDiscoveryRoutineContext(items, heatStyling)`, and passes it to `loadDiscoveryIdealRoutine(admin, userId, intakeId, { routineOverride })`. **Shared scanner code stays untouched**: `loadDiscoveryIdealRoutine` calls `prepareScannerContext` exactly as today; only when an override exists AND `context.snapshotSource === "initial"` (a refined participant keeps her real Feinschliff answers) does it recompute inside the discovery module: `computeNeedPlan({ rawEnvelope: context.snapshot.sourceQuiz, artifactId: context.snapshot.profile.source.artifactId, projection: "initial_quiz", computationVersion: context.snapshot.computationVersion, createdAt: context.snapshot.createdAt, routine: routineOverride })`. `projection` stays `"initial_quiz"` because `"refined_post_plan"` returns `needs_clarification` when shampoo frequency is unknown (`src/lib/personal-plan/compute-stage1.ts:76`), and decisions read the routine regardless of projection (`src/lib/personal-plan/input.ts:189-217`; only `compute-stage1.ts` and `stage2-entry-adapter.ts` branch on projection). A `status !== "ready"` result falls back to the unmodified context (never an error). The previews (`computeStage1ProductExamplePreviews`) run on the resulting snapshot, so steps and previews stay consistent. `ScanEvaluationContext.snapshotSource` keeps its `"initial" | "refined"` contract (Codex new P1: `src/lib/scan/profile-context.ts:11` + scanner publication CHECK/RPC); the discovery origin is a discovery-only field `routineSource: "quiz_only" | "intake_answers"` on `DiscoveryIdealRoutineResult`. Nothing persists: no publisher/persistence service on this path (test asserts only the existing reads).
...
## 6b. Codex confirm pass → Rev. 3

Resolved in Rev. 2: P1-2, P1-4, P1-5, P2-7, P2-8. Rev. 3 fixes the rest (each verified against code by the main session):
- P1-1: recompute with `projection: "initial_quiz"` + routine override inside the discovery module (refined projection would `needs_clarification` on unknown shampoo frequency); shared `prepareScannerContext` untouched; non-ready → fall back.
- New P1: no new `snapshotSource` value; discovery-only `routineSource` field.
- P1-3: stated concern resolved from raw answers; narrative gets it explicitly with copy for the four non-legacy codes; lane uses it only when legacy-mappable.
- P1-6 (D2): styling marker preserved through participant + cockpit projections and branched before `category_unknown`.
No further plan-review pass: remaining risk is implementation-level and covered by the named tests and the one whole-branch Codex review per PR (lean rule).
