...
- Deterministic red/green: original safe-context stub failed all three source/privacy cases; review regression failed both same-journey response-order cases before the identity-aware fix. Final focused and shared analytics suites pass: 94 tests, zero failures or skips. TypeScript passes; configured source lint passes (repository ignores scripts/tests).
...
- GitHub review identified an additional HTTP-200 partial-context case on database lookup failure. An explicit `analyticsContextReady` signal now causes bounded acquisition retries; scanner page capture is suppressed after exhaustion while signed identity remains available to existing UI callers. Three red/green regressions cover server signal, cache/retry, and event suppression/recovery. The isolated browser transport check also starts with an incomplete response and requires recovered campaign properties. This does not reconstruct legacy events or change other destination event definitions.
