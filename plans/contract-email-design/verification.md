...
## Final merge review correction
Automated PR review found that the dispatcher marked the send as started before the new async PDF preparation. An unsupported-character failure was therefore misclassified as uncertain delivery. Reproduced with an actual PDF encoding failure and zero fetch calls, then fixed with a preparation-only error type caught distinctly by the dispatcher. The provider send stays outside that catch, retaining HTTP/ambiguous error semantics. Added exact outcome assertions for all error classes. Nick subsequently authorized merge and deployment; final GitHub checks and production verification are still required.
