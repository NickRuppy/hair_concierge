...
Unit: T2–T5, T8, T9. Playwright: `tests/scan-funnel-journey.spec.ts` (`/lp/scan` mit Flag → Quiz mit drei Einfügungen → Lead → Angebot `scan-regal-v1` → **Zahlungs-Ersatz** (Ruling 2026-09-12: es gibt keinen CI-tauglichen Test-Mode-Kaufpfad; die Spec seedet die Kauf-Korrelation per Service-Role und meldet sich wie `tests/quiz-onboarding-e2e.spec.ts` an — Karte/PayPal-Aktivierung bleiben durch die Unit-Tests der Aktivierungspfade abgedeckt) → Bereitstellung → `/scan` → echtes Scan-Ergebnis via `/api/scan/resolve` (EAN 3574661799438, HTTP 200 + Ergebnis), Fixture = Neukunde ohne Bedarfs-Snapshot; läuft nur in der manuellen full_ci-Lane mit echten Secrets). Test in die CI-Browser-Kommandos (`package.json` `test:e2e*`) **und** die Mobile-WebKit-Allowlist (`playwright.config.ts:37`) eintragen. Bestehende Legacy-Suiten unverändert grün. `npm run funnel:check`, `npm run test:node`, `npm run ci:verify`, `npm run test:chat` (Regression). Manuell auf dem Worktree-Dev-Server (`dev:worktree`) komplett durchklicken, mobil 375 px, inkl. Back-Navigation und Reload auf jeder Einfügung.
...
**Varianten (Stand nach Umsetzung, 2026-09-12):** Einfügung 1 zeigt OGX (Haardicke „dick“): feines Haar → „Passt nicht zu deinem Haar“, mittleres → „Passt mit Einschränkung“, dickes → „Passt zu deinem Haar“. Einfügung 2 spiegelt das Foto (Alverde Balance, Kopfhaut „trocken, gereizt“): Kopfhaut trocken oder gereizt → grün; ausgeglichen, fettig oder Schuppen → „Passt nicht zu deiner Kopfhaut“ (Ruling „Karte spiegelt den Screen“; das frühere „Schuppen → grün“ ist damit überholt). Einfügung 3 (Balea Repair Kur): Pflegegewicht aus der Haardicke, feines Haar → „mit Einschränkung“. Zurück-Navigation über Einfügungen funktioniert wie zwischen Fragen (Browser-Back inklusive).
