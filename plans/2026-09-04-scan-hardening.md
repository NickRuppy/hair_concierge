...
**PR 2 (client side, tasks 8-12) — implemented 2026-09-05 on `codex/scan-hardening-client` (stacked on PR 1).** SDD per-task reviews clean (two fix rounds on the scanner hook, one on the flow); Codex whole-branch review (effort high) → 3 Important + 2 Minor, all fixed in one wave and re-reviewed clean. `ci:verify` green; 812 node tests; Playwright `tests/scan-flow.spec.ts` 11/11 on the `/labs/scan` harness (fake camera + fake detector, page.route-mocked APIs), wired into the journey CI job; the real `/scan` page driven via dev login (denied-camera tile → retry → no sheet re-pop; manual barcode → toast → back to scanning). Structure: `scanner.tsx` 536 → 158 lines (UI only) + `use-scanner-loop.ts` (lifecycle) + pure `scanner-loop.ts`/`scanner-session.ts`; `scan-flow.tsx` on the pure reducer `scan-flow-state.ts` with request tokens; `useLatestRequest` shared by resolve/submit/save/search/wishlist. Rulings beyond §3: a blocked (D6) read resets the fallback clock and the hint counter instead of firing; a foreground track `mute` gets a 3 s grace before recovery and recovery acquires the new stream before releasing the old; a decode the flow refuses is un-fired so the barcode re-fires after the sheet closes; `scan_submission_created` is tracked whenever the server created the row; a save dismissed mid-flight usually shows no confirmation. Parked: a failed decoder (wasm) load shows the "keine Kamera" notice (copy imprecise for that rare cause); WebKit lifecycle Playwright project; splitting `use-scanner-loop.ts` recovery/stream ownership into a helper.
