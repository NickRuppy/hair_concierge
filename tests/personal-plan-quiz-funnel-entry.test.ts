...
test("personal-plan quiz UI reflects the approved visual journey constraints", () => {
  ...
  assert.match(quiz, /preloadImage=\{config\.field === "texture" && optionIndex === 0\}/)
  assert.match(quiz, /fetchPriority=\{preloadImage \? "high" : "auto"\}/)
  assert.match(quiz, /preload=\{preloadImage\}/)
  assert.doesNotMatch(quiz, /\bpriority(?:=|\s*$)/m)
