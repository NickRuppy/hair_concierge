...
function manifest() {
  return buildPersonalPlanProductDispositionReversalManifest({
    ...
    items: Object.entries(PRODUCT_DISPOSITION_REVERSAL_PRODUCTS).map(
      ([product_id, reason_code]) => ({
        product_id,
        expected_product: {
...
async function database(publicationReady = true) {
  const pg = new PGlite()
  await pg.exec(`
    CREATE ROLE anon;
  ...
  await pg.exec("BEGIN")
  for (const [productId, reasonCode] of Object.entries(PRODUCT_DISPOSITION_REVERSAL_PRODUCTS)) {
    await pg.query(
    ...
    await pg.query(
      `INSERT INTO public.personal_plan_product_search_dispositions
       ...
       VALUES ($1, 'retired_from_personal_plan', $2, 'Old reviewed reason',
...
test("one drifted prior disposition rolls back the whole seven-product reversal", async (t) => {
  ...
  await pg.query(
    `UPDATE public.personal_plan_product_search_dispositions
     ...
     WHERE product_id = $1`,
    [Object.keys(PRODUCT_DISPOSITION_REVERSAL_PRODUCTS)[6], "0".repeat(64)],
  )
...
test("production deferred curated-publication trigger rejects a direct incomplete unquarantine", async (t) => {
  const pg = await database(false)
  t.after(async () => pg.close())
  const [productId] = Object.keys(PRODUCT_DISPOSITION_REVERSAL_PRODUCTS)
