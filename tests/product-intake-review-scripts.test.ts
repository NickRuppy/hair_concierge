...
test("destructive review scripts default to dry-run and require explicit confirmation", () => {
  ...
  assert.match(notifyPendingScript, /Dry-run only/)
  assert.match(notifyPendingScript, /--apply --confirm/)
  assert.match(notifyPendingScript, /Notify-pending writes require --confirm/)
