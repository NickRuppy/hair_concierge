 ...
 *
 * The golden file was written from `ScanResultCard` as it stood BEFORE the extraction
 * (`scripts/dev/capture-scan-result-card-parity.ts`, run once at
 * `0e8d57e3`) and is committed with it. Afterwards, the card composes the two extracted
 * pieces — and every byte of its markup must still match, in every state. A fragment that
 * became a wrapper `div`, a lost class, a reordered section or a dropped attribute all
 * fail here rather than in Nick's hand on the next scan.
 *
 * Regenerating the golden is not a way to make this pass: a diff here means the scan sheet
 * changed, which this branch is not allowed to do.
 */
