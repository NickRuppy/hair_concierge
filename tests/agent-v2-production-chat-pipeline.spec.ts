...
test("AgentV2 production pipeline resolves my category product from matched routine inventory for product facts", async () => {
  const hairProfile = createCompleteHairProfile({
    current_routine_products: ["shampoo", "conditioner", "leave_in"],
  })
  const leaveInProduct = createProduct("aussie-100h-leave-in")
  leaveInProduct.name = "AUSSIE SOS Leave-In Haarserum 100 Hours Hydration"
  leaveInProduct.brand = "AUSSIE"
  ...
  const selection: SelectProductsToolResult = {
    projection: {
      ...
      decision: "recommended",
      product_response_policy: "recommend_with_caveat",
      policy_reason: "Matched current routine product assessment.",
      profile_basis: ["Feines, welliges Haar."],
      category_guidance: "Leave-in passt als konkrete Produktbewertung.",
      products: [
        {
          rank: 1,
          product_id: leaveInProduct.id,
          name: leaveInProduct.name,
          brand: leaveInProduct.brand,
          price_eur: leaveInProduct.price_eur,
          currency: leaveInProduct.currency,
          fit_reason: "Kann sparsam in den Laengen sinnvoll sein.",
          caveat: null,
          supported_claims: [],
          unsupported_requested_signals: [],
        },
    ...
    products: [leaveInProduct],
    effectiveHairProfile: hairProfile,
    runtime: buildRecommendationEngineRuntimeForChat({
      hairProfile,
      routineItems: [],
      productCategory: "leave_in",
      message: "passt mein leave-in zu mir",
    }),
  }
  ...
  await runAgentV2ProductionPipeline(
    {
      ...
      conversationId: "conversation-1",
      userId: "user-1",
      requestId: "request-current-routine-leave-in-facts",
      productIntakeEnabled: true,
    },
    {
      verifyConversationOwnership,
      ...
      getUserContext: async () => ({
        ...
        routine_inventory: [
          {
            category: "leave_in",
            product_name: "Leave-In Haarserum 100 Hours Hydration",
            frequency_range: "weekly_1x",
            brand_text: "AUSSIE",
            product_id: leaveInProduct.id,
            product_submission_id: "submission-aussie",
      ...
      createSelectProductsTool:
        (options = {}) =>
        async (input: SelectProductsToolParams) => {
          assert.deepEqual(input.targetProductIds, [leaveInProduct.id])
          assert.deepEqual(input.targetProductHints, [
            {
              product_id: leaveInProduct.id,
              name: "AUSSIE Leave-In Haarserum 100 Hours Hydration",
              category: "leave_in",
            },
          ])
          options.onResult?.(selection)
      ...
      runAgentV2ResponsesTurn: async (params) => {
        ...
        loadProductFactsResult = await params.tools.load_product_facts({
          category: "leave_in",
          reason: "The user asks whether their current leave-in suits them.",
          user_request: "passt mein leave-in zu mir",
          evidence_quote: "mein leave-in",
        })
        return createAgentV2Result()
      },
    },
  ...
  assert.deepEqual(receivedActiveResolvedProductContext, {
    ...
    name: "AUSSIE Leave-In Haarserum 100 Hours Hydration",
    category: "leave_in",
    original_user_message: "passt mein leave-in zu mir",
  })
  assert.equal(
    (loadProductFactsResult as { products?: Array<{ product_id?: string }> }).products?.[0]
      ?.product_id,
