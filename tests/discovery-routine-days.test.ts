 ...
 * products into day cards — Waschtag, Intensiv-Pflegetag, Zwischendurch, Styling, Weitere —
 * only the non-empty ones, in that order. Pure and client-safe.
 */
...
test("an empty list composes no days; „none“ rows never appear", () => {
  assert.deepEqual(composeDiscoveryRoutineDays([]), [])
  assert.deepEqual(composeDiscoveryRoutineDays([item({ source: "none", category: "mask" })]), [])
})

test("every category lands on its day, in the card's own order, only non-empty cards", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      ["between", "Zwischendurch", null, ["finish", "dry", "scalp"]],
...
test("Waschtag cadence = the MOST frequent shampoo; unknown or unasked → no cadence", () => {
  ...
  assert.deepEqual(shape(noShampoo), [["wash_day", "Waschtag", null, [noShampoo[0].items[0].id]]])
})
...
test("Intensiv-Pflegetag cadence = the most frequent mask or bondbuilder", () => {
  ...
  assert.equal(days[0].kind, "intensive_day")
  assert.equal(days[0].cadence, "biweekly_1x")
  assert.equal(days[0].cadenceLabel, "Alle 2 Wochen")
})
...
test("a legacy role-less oil sits in Zwischendurch; items keep their own objects", () => {
  const oil = item({ id: "oil", category: "oil", usageRole: null })
  const days = composeDiscoveryRoutineDays([oil])
  assert.equal(days[0].kind, "between")
  assert.equal(days[0].items[0], oil)
})
