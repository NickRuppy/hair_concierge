...
test.describe("Personal Plan products lab", () => {
  ...
  test("keeps the mobile-first product journey customer-facing while reaching the routine handoff", async ({
    ...
    await expect(page.getByRole("heading", { name: "Wie gut passt dein Produkt?" })).toBeVisible()
    ...
    await page.getByRole("button", { name: /als Ersatz übernehmen/ }).click()
    await expect(page.getByText("Oil Length Seal", { exact: true })).toBeVisible()
    await page.getByRole("button", { name: /Oil Length Seal weiterverwenden/ }).click()
    await page.getByRole("button", { name: /Oil Length Seal weiterverwenden/ }).click()
    await page.getByRole("button", { name: /Oil Length Seal weiterverwenden/ }).click()
