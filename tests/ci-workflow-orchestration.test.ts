...
test("the Stage 3 CI browser suite isolates the production lab from development journeys", () => {
  ...
  assert.match(
    journeyCommand,
    /playwright test tests\/personal-plan-start\.spec\.ts tests\/personal-plan-stage2-refinement\.spec\.ts tests\/personal-plan-stage1-2-3\.spec\.ts tests\/personal-plan-application-transition\.spec\.ts tests\/personal-plan-routine-editor\.spec\.ts --project=chromium/,
  )
