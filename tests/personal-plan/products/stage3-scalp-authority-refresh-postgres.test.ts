...
test("Scalp v3 migration executes both preceding and current runtime refreshes", async (t) => {
...
`)
  ...
  const oldPayload = {
    authoritySnapshot: {
      ...
      coverage: [],
      orderedCategories: ["shampoo", "mask", "scalp_care"],
  ...
  const oldCursor = {
    categoryCursor: "mask",
    completedCaptureCategories: ["shampoo", "mask", "scalp_care"],
    completedDecisionKeys: ["old-decision"],
  }
  ...
  unsupportedPayload.authoritySnapshot.authorityVersions.scalp_care = "personal-plan.scalp-care.v4"
