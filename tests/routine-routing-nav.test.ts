...
test("middleware app-route helper matches Anwendung, chat, routine, scan, and tracker by route segment", () => {
  ...
  assert.equal(isAuthenticatedAppRoutePath("/scan"), true)
  assert.equal(isAuthenticatedAppRoutePath("/scan/history"), true)
  ...
  assert.equal(isAuthenticatedAppRoutePath("/scan-ish"), false)
  ...
  assert.equal(isAuthenticatedAppRoutePath("/api/scan"), false)
...
test("middleware subscription helper requires access for Anwendung and existing app paths", () => {
  ...
  assert.equal(requiresSubscriptionPath("/scan"), true)
  assert.equal(requiresSubscriptionPath("/scan/history"), true)
  assert.equal(requiresSubscriptionPath("/api/scan"), true)
  assert.equal(requiresSubscriptionPath("/api/scan/resolve"), true)
  ...
  assert.equal(requiresSubscriptionPath("/scan-ish"), false)
  assert.equal(requiresSubscriptionPath("/api/scan-ish"), false)
