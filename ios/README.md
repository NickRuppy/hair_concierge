# Chaarlie native scanner — existing-user profile editing

## Research delivery (Slice B)

Research links use `https://chaarlie.de/app/research/{submissionId}` and the current build's `chaarlie-local`, `chaarlie-pilot`, or Release `chaarlie` scheme with `://research/{submissionId}`. The app validates the exact URL shape, preserves a signed-out destination through login/profile completion, and loads the current owner-bound assessment from `GET /api/mobile/v1/scan/research-result/{submissionId}`. A link is never authentication. Missing, pending and failed results provide a retry; explicit logout clears the destination. The HTTPS landing page's button is the working scheme fallback; Universal Link association still requires real Apple signing/domain setup.

The optional bootstrap `researchDeliveryEnabled` flag defaults off for old servers. Push also requires `ChaarliePushEnvironment` (`sandbox` or `production`) in the configured device build, a matching real `aps-environment` entitlement, APNs credentials/topic on the backend and a successful device-token callback. None of those signing capabilities are asserted in the project. Simulator builds always disable system registration/permission. Only a confirmed pending research submission requests alert permission, after APNs registration succeeds; denial does not change the research request or email delivery. Existing authorized sessions refresh token binding when the app becomes active.

Registration uses a persisted installation UUID, rotated on logout/account switch. Logout clears local state immediately and attempts owner-bound server revocation with the captured old token/installation ID; offline revocation is best-effort, with backend lease expiry as the fallback. Late permission and result responses cannot restore state or bind to another session. Physical-device APNs receipt, cold notification launch, Universal Link delivery and a signed public build remain release checks; simulator unit tests prove only the app routing and coordination behavior.
...
Scanner capture runs on one serial queue. The detector disarms on first read, while a request, result or retry is active, and re-arms after dismissal. Backgrounding, leaving Scan and opening manual search stop capture. A partially visible result preserves its preview and pauses detection. Requests retain the scanned identifier on network failure. Search uses request identities to ignore stale responses; account changes invalidate profile, search and scanner results. Profile edits use the owner-bound mobile API; there are no native routine/billing requests.

The UI uses the approved German product header, main comparison table, verdict and factual mismatch summary; all authority-backed alternative rows remain available in the horizontal carousel. Main and alternative purchase buttons independently validate their own public HTTP(S) URLs (catalog images require HTTPS). Missing main shop links omit the entire footer. Profile displays persisted answers, the complete prefilled ten-question editor and logout. Cancel discards the in-memory draft; save atomically publishes profile and scanner context, preserves paid routines, and invalidates prior scan/search completions. A lost response retries the same request UUID; a conflicting revision requires explicit reload. Account changes discard drafts and reject late responses. Delete/onboarding/research controls remain absent. The missing-product close fallback is explicitly development-only and does not constitute public-release research behavior.

Profile editing adds service-only migration `20260916053907_mobile_profile_edit.sql` and GET/POST `/api/mobile/v1/profile/edit`. Apply it only to the separately authorized target stack before activating this build. The parent/setup tasks own local stack, device proxy and phone activation; this branch contains only a [proxy patch](../plans/ios-scanner/profile-edit-device-proxy.patch) against the reviewed device-QA worktree. See the [profile-edit plan](../plans/ios-scanner/profile-edit-plan.md) and its verification receipt for the exact tested tree and remaining physical-device checks. `ChaarlieTests/Fixtures/profile-edit-v1.json` is byte-identical to the server wire fixture under `tests/fixtures/mobile/`.
...
Both use SIL Open Font License 1.1. Do not remove the licenses from distributions. Dynamic Type scales the fonts and switches comparison rows to vertically labeled values at accessibility sizes; no fixed-row clipping. Reduce Motion disables the alternatives fade. VoiceOver combines each comparison row and names each product's shop destination. Standard and largest-text simulator screenshots were inspected; targeted XCTest accessibility audits passed for element detection, sufficient descriptions and traits, with explicit comparison-value/target and product-specific shop-label assertions. Opening the fixture shop in system Safari and returning to the same native result also passed; merchant-page success is not claimed. See the implementation receipts for actual connected and device evidence; simulator camera fallback does not prove physical barcode capture. App Store submission, production config, associated domains, onboarding, deletion and research delivery are later gates.
...
## Single-property Profile editing

The [approved single-edit addendum](../plans/ios-scanner/profile-single-edit-spec.md) makes each grouped Profile row editable in a native SwiftUI sheet. Save is explicit and requires a changed valid draft; the secondary “Alle Angaben überprüfen” retains the complete review. Inline help uses the shared quiz DTO; surface/elasticity instructions are immediately visible. Sheets/toolbars use standard system components, Chaarlie typography, Dynamic Type and iOS18-compatible APIs, built with Xcode26.6. No custom glass material or deployment-target uplift is needed.

One property still sends the complete authoritative snapshot through the existing atomic edit API. A defensive snapshot without hair length asks only for that explicit answer in the same sheet; this synthetic test case does not claim a reachable affected account or weaken backend admission. The native delta needs no migration or proxy change. See the current profile-edit verification receipt for exact test evidence and fingerprint; integration and physical-device installation remain setup-owned.
