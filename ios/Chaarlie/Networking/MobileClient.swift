...
enum MobileError: Error, Equatable {
    case configuration, unauthorized, invalidResponse, unavailable, stale, invalidCode
}
...
struct MobileConfiguration: Sendable {
    ...
    init(baseURL: URL) throws {
        ...
        #else
        // Distribution hosts and associated domains have a separate approval/verification gate.
        throw MobileError.configuration
        #endif
    ...
    static func current() throws -> Self {
        #if DEBUG
        let value = ProcessInfo.processInfo.environment["CHAARLIE_API_BASE_URL"] ?? "http://127.0.0.1:3218/api/mobile/v1"
        guard let url = URL(string: value) else { throw MobileError.configuration }
...
actor MobileClient {
    ...
    init(configuration: MobileConfiguration, transport: any HTTPTransport = URLSessionTransport(),
         store: any SessionPersistence = KeychainSessionStore()) {
        self.configuration = configuration
        self.transport = transport
        self.store = store
        self.session = try? store.loadSession()
    }
    ...
    func verify(attemptId: String, code: String? = nil, tokenHash: String? = nil) async throws -> MobileSession {
        guard session == nil else { throw MobileError.stale }
        let expected = epoch
        var body = ["attemptId": attemptId]
        if let code { body["code"] = code }
        if let tokenHash { body["tokenHash"] = tokenHash }
        let verified: MobileSession = try await decode(raw("auth/verify", method: "POST", body: body))
        try install(verified, ifGeneration: expected)
        return verified
    }
