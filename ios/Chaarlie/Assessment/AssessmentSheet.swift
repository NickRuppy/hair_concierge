...
struct AssessmentSheet: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                if let product = result.product {
                    HStack(alignment: .top, spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(product.name).font(ChaarlieTheme.body(15, weight: .bold)).fixedSize(horizontal: false, vertical: true)
                            Text(product.detail).font(ChaarlieTheme.body(12)).foregroundStyle(ChaarlieTheme.muted).fixedSize(horizontal: false, vertical: true)
                ...
                if result.kind == .assessment {
                    Text(result.verdictTitle ?? result.verdictLabel ?? "")
                        .font(ChaarlieTheme.display()).foregroundStyle(result.verdict == .supportive ? Color(hex: 0x805a16) : ChaarlieTheme.ink)
                    ...
                    Text(result.mismatchSummary ?? AssessmentPresentation.deviation(result.rows ?? []))
                        .font(ChaarlieTheme.body(15)).fixedSize(horizontal: false, vertical: true)
                    ...
                    if let alternatives = result.alternatives, !alternatives.isEmpty {
                        Divider()
                        Text("Alternativen").font(ChaarlieTheme.display(22)).accessibilityAddTraits(.isHeader)
                        Text((result.verdict == .ideal ? "Passen auch zu deinem Haar" : "Passen gleich gut oder besser") + " · \(alternatives.count)")
                            .font(ChaarlieTheme.body(13)).foregroundStyle(ChaarlieTheme.muted)
    ...
    @ViewBuilder private var unavailableContent: some View {
        if result.kind == .submission_required {
            #if DEBUG
            Text("Für dieses Produkt liegt noch keine Einschätzung vor.").font(ChaarlieTheme.display())
            Text("Entwicklungsbuild: Die Produktaufnahme folgt in einem späteren Schritt.")
                .font(ChaarlieTheme.body(13)).foregroundStyle(ChaarlieTheme.muted)
            #else
            Text("Produkt derzeit nicht verfügbar.").font(ChaarlieTheme.display())
            #endif
        } else {
            Text(result.headline ?? "Noch nicht einschätzbar").font(ChaarlieTheme.display())
            if let subtitle = result.subtitle { Text(subtitle) }
            ForEach(result.reasons ?? [], id: \.self) { Text($0).font(ChaarlieTheme.body()) }
...
struct AlternativeCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                Text(alternative.product.name).font(ChaarlieTheme.body(14, weight: .bold)).fixedSize(horizontal: false, vertical: true)
                if revealed {
                    Text(alternative.verdictTitle)
                        .font(ChaarlieTheme.display(18)).foregroundStyle(alternative.verdict == .supportive ? Color(hex: 0x805a16) : ChaarlieTheme.ink)
                    Text(alternative.mismatchSummary).font(ChaarlieTheme.body(13))
                    Text(alternative.product.detail).font(ChaarlieTheme.body(12)).foregroundStyle(ChaarlieTheme.muted)
    ...
    var body: some View {
        VStack(spacing: 0) {
            if !typeSize.isAccessibilitySize {
                ...
                }.font(ChaarlieTheme.body(12, weight: .bold)).padding(.horizontal, 10).padding(.vertical, 9)
                    ...
                    Group {
                        if typeSize.isAccessibilitySize {
                            VStack(alignment: .leading, spacing: 8) {
                                HStack { status(row); Text(GermanLineBreaks.text(row.label)).font(ChaarlieTheme.body(15, weight: .bold)); Spacer(); Image(systemName: "info.circle") }
                            ...
                            HStack(spacing: compact ? 6 : 8) {
                                Text(row.label.replacingOccurrences(of: "Pflegegewicht", with: "Pflege-\ngewicht").replacingOccurrences(of: "Pflegerichtung", with: "Pflege-\nrichtung"))
                                    .font(ChaarlieTheme.body(13, weight: .semibold))
                                    .frame(width: 68, alignment: .leading)
                                status(row)
                                Text(row.productValue ?? "–").foregroundStyle(color(row)).font(ChaarlieTheme.body(compact ? 12 : 13, weight: .bold))
                                    .frame(maxWidth: .infinity, alignment: .leading).fixedSize(horizontal: false, vertical: true)
                                Text(row.targetValue ?? "–").foregroundStyle(ChaarlieTheme.plum).font(ChaarlieTheme.body(compact ? 12 : 13, weight: .semibold))
                                    .frame(maxWidth: .infinity, alignment: .leading).fixedSize(horizontal: false, vertical: true)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(GermanLineBreaks.text(row.label))
                ...
                .font(ChaarlieTheme.display(22))
                .modifier(GlossaryLineHeight(size: 22, multiple: 1.2, fontName: "PlayfairDisplay-Regular", relativeTo: .title))
...
private struct GlossaryList: View {
    ...
    private func label(_ stop: ScanRow.Stop) -> some View {
        Text(GermanLineBreaks.text(stop.label)).font(ChaarlieTheme.body(15, weight: .semibold))
    ...
    private func meaning(_ stop: ScanRow.Stop) -> some View {
        Text(stop.displayMeaning).font(ChaarlieTheme.body(13))
...
private struct GlossaryLineHeight: ViewModifier {
    ...
    init(size: CGFloat, multiple: CGFloat, fontName: String = "PlusJakartaSans-Regular", relativeTo: Font.TextStyle = .body) {
        let nativeHeight = (UIFont(name: fontName, size: size) ?? .systemFont(ofSize: size)).lineHeight
