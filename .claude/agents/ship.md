...
Note: this tests the _currently deployed_ version at the production URL as a smoke check.
...
This `/ship` pipeline stops after commit, push, and PR creation. It never infers merge, cleanup, deployment, or production authorization.

After `/ship`, an explicit **“merge it”** in the main session means:

1. refresh final PR, check, review, migration, and content-fingerprint state and bind the squash merge to the reviewed head with `gh pr merge <number> --squash --match-head-commit <reviewed-head-sha>`;
2. verify the merged PR and merge SHA;
3. from the primary root `main`, dry-run `npm run worktree:finish -- --pr <number>` and apply it when every guard passes;
4. report root sync and exact remote/local/worktree postconditions.

An explicit **“merge but keep the worktree until <condition>”** skips the finisher, locks the worktree with the stated reason, retains its local branch, and records the release condition. The merged remote branch is deleted only when no dependent PR or collaborator still needs it. Merge never implies deployment or production writes.
