-- Day-after collection: the first charge lands in the provider's daily batch
-- after the trial end, and access bridges that bounded window. recover_unpaid
-- inside the window would let a customer cancel their own pending first
-- collection and re-purchase, so for uncancelled trials the offer AND the
-- authoritative begin/guard paths wait until the window has closed (next UTC
-- midnight after the trial end, plus two days — mirroring
-- paypalTrialCollectionWindowEnd). A trial cancelled at period end has no
-- pending collection: its recovery eligibility stays at the trial end.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path='' AS $$
 ...
 AND (e.cancel_at_period_end
   OR (date_trunc('day', e.original_trial_end_at AT TIME ZONE 'UTC') + interval '3 days') <= (clock_timestamp() AT TIME ZONE 'UTC')) THEN 'recover_unpaid'
...
LANGUAGE sql SECURITY DEFINER SET search_path='' AS $$
 ...
 AND ((o.kind='recover_unpaid' AND e.first_payment_succeeded_at IS NULL AND e.original_trial_end_at<=clock_timestamp()
   AND (e.cancel_at_period_end
     OR (date_trunc('day', e.original_trial_end_at AT TIME ZONE 'UTC') + interval '3 days') <= (clock_timestamp() AT TIME ZONE 'UTC')))
...
BEGIN
 ...
 OR (p_kind='recover_unpaid' AND (e.first_payment_succeeded_at IS NOT NULL OR e.original_trial_end_at>clock_timestamp()
   OR (NOT e.cancel_at_period_end
     AND (date_trunc('day', e.original_trial_end_at AT TIME ZONE 'UTC') + interval '3 days') > (clock_timestamp() AT TIME ZONE 'UTC'))))
