-- Dry Shampoo fit is determined by refresh effect, color fit, scalp fit,
-- format, and executable use. Hair-shaft thickness is not an applicable fact.
...
AS $function$
  WITH catalog_roles AS (
    ...
    SELECT
      ...
      END
      AND (
        p.category_key IN ('heat_protectant', 'dry_shampoo')
...
BEGIN
  ...
  v_has_facts := v_has_facts AND (
    v_product.category_key IN ('heat_protectant', 'dry_shampoo')
