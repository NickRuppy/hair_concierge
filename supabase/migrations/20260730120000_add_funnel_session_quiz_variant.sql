-- Persist the registered quiz experience shown for every funnel journey.
-- Do not widen this mapping without an explicit migration: historical rows
-- need a deterministic, reviewable backfill value.
...
BEGIN
  IF EXISTS (
    ...
    WHERE package_key NOT IN (
      'default_organic',
      'meta_routine_v1',
      'scalp_check_placeholder',
      'meta_personal_plan_v1'
    )
  ) THEN
    RAISE EXCEPTION
      'Cannot backfill funnel_sessions.quiz_variant: an unreviewed package_key exists';
  END IF;
END
$guard$;

ALTER TABLE public.funnel_sessions
  ADD COLUMN quiz_variant text;

UPDATE public.funnel_sessions
SET quiz_variant = CASE package_key
  WHEN 'meta_personal_plan_v1' THEN 'personal-plan-quiz-v1'
  ELSE 'legacy-quiz-v1'
END
...
COMMENT ON COLUMN public.funnel_sessions.quiz_variant IS
  'Immutable registered quiz experience shown for this journey.';

DROP FUNCTION public.record_funnel_event(
...
BEGIN
  ...
  VALUES (
    ...
    p_offer_variant,
    CASE
      WHEN p_quiz_variant IS NOT NULL THEN p_quiz_variant
      WHEN p_package_key = 'meta_personal_plan_v1' THEN 'personal-plan-quiz-v1'
      ELSE 'legacy-quiz-v1'
    END,
    p_entry_path,
    p_entry_url,
