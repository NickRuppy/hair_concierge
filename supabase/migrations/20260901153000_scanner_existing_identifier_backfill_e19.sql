-- Extend the existing guarded executor with the exact reviewed E19 cohort.
-- Applied E1-E18 migrations and fingerprints remain unchanged.
...
ALTER TABLE public.scanner_identifier_backfill_batches
  ...
  CHECK (batch_name IN ('E1', 'E2', 'E3', 'E4', 'E5', 'E6', 'E7', 'E8', 'E9', 'E10', 'E11', 'E12', 'E13', 'E14', 'E15', 'E16', 'E17', 'E18', 'E19'));
...
BEGIN
  ...
  IF v_batch->>'schema_version' IS DISTINCT FROM 'scanner-existing-identifier-backfill-v1'
     OR v_batch->>'batch' NOT IN ('E1', 'E2', 'E3', 'E4', 'E5', 'E6', 'E7', 'E8', 'E9', 'E10', 'E11', 'E12', 'E13', 'E14', 'E15', 'E16', 'E17', 'E18', 'E19')
  ...
  ELSIF v_batch_name = 'E17' THEN
    v_expected_products := 1;
    v_expected_gtins := 1;
    v_approved_fingerprint := '6b259ee2ceff31116e92d04a5a2c627379eb4b88e8cde3c51ae026860243f5ce';
  ELSIF v_batch_name = 'E18' THEN
    v_expected_products := 14;
    v_expected_gtins := 14;
    v_approved_fingerprint := '1b59aefef8ba0a5ae217c16d49a37b2b1e2e118157855a68b7c2e2931d3d5643';
  ELSIF v_batch_name = 'E19' THEN
    v_expected_products := 1;
    v_expected_gtins := 3;
  ...
  LOOP
    ...
    END IF;
    -- E17 retains the strict scan-result-readiness boundary until the newly
    -- verified conditioner protocol has resolved the prior disposition.
    IF v_batch_name = 'E17' AND EXISTS (
      SELECT 1
      FROM public.personal_plan_product_search_dispositions disposition
      WHERE disposition.product_id = v_product_id
    ) THEN
      RAISE EXCEPTION 'scanner identifier backfill E17 product is not scan-result-ready';
    END IF;
    -- E18 retains the strict scan-result-readiness boundary for every repaired
    -- Oil until its obsolete product-level disposition has been released.
    IF v_batch_name = 'E18' AND EXISTS (
      SELECT 1
      FROM public.personal_plan_product_search_dispositions disposition
      WHERE disposition.product_id = v_product_id
    ) THEN
      RAISE EXCEPTION 'scanner identifier backfill E18 product is not scan-result-ready';
    END IF;
    -- E19 keeps Balea unresolvable until its exact everyday Shampoo protocol
    -- has replaced the prior awaiting-analysis disposition.
    IF v_batch_name = 'E19' AND EXISTS (
      SELECT 1
      FROM public.personal_plan_product_search_dispositions disposition
      WHERE disposition.product_id = v_product_id
    ) THEN
