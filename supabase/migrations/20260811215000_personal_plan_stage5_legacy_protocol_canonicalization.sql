-- Canonicalize the legacy Heat/Scalp protocol rows that predate the exact
-- guidance payload. A legacy row can only be upgraded from NULL guidance;
-- established canonical authority remains immutable and conflict-guarded.
...
BEGIN
  ...
  LOOP
    ...
    IF jsonb_typeof(v_payload) IS DISTINCT FROM 'object'
       ...
       OR coalesce(pg_catalog.btrim(v_item->>'source_label'), '') = ''
       OR coalesce(pg_catalog.btrim(v_item->>'source_text'), '') = ''
       OR coalesce(v_item->>'source_url', '') !~ '^https?://'
       OR NOT EXISTS (
         SELECT 1
         FROM pg_catalog.jsonb_array_elements(v_payload->'evidence') evidence
         WHERE evidence->>'sourceUrl' = v_item->>'source_url'
       ) THEN
    ...
    IF FOUND THEN
      IF v_existing.guidance_payload IS NULL THEN
        UPDATE public.product_application_protocols AS protocol
        SET cadence = v_item->'cadence',
            application_stage = v_payload#>>'{sequence,anchor}',
            application_state = CASE v_payload->>'applicationFamily'
              WHEN 'pre_heat_damp' THEN 'damp'
              WHEN 'pre_heat_dry' THEN 'dry'
              WHEN 'either_state_protection' THEN 'either'
              ELSE NULL
            END,
            placement = v_payload#>>'{protocolFacts,applicationArea}',
            contact_time_seconds = NULLIF(v_payload#>>'{protocolFacts,contactTimeSeconds}', '')::integer,
            rinse_action = v_payload#>>'{protocolFacts,rinse}',
            reapplication = CASE v_payload#>>'{protocolFacts,reapplication}'
            ...
            guidance_payload = v_payload
        WHERE protocol.product_id = v_product_id
          AND protocol.category = v_category
          AND protocol.role = v_role
          AND protocol.guidance_payload IS NULL;
        IF NOT FOUND THEN
          RAISE EXCEPTION 'Stage 5 protocol legacy authority changed during canonicalization: %', v_product_key;
        END IF;
      ELSIF v_existing.guidance_payload IS DISTINCT FROM v_payload
