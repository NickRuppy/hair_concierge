-- Recompile the exact approved Oil executor with column precedence for SQL
-- expressions such as ON CONFLICT (product_id, ...). The function's
-- RETURNS TABLE output exposes product_id as a PL/pgSQL variable, which would
-- otherwise make that conflict target ambiguous at runtime.
