-- Idempotently normalize the narrow legacy Stage-4 initial shape left by the
-- older completion path: no active Routine, a pending proposal with no base,
-- and a candidate linked to the plan's current refined source.
