-- Extend the guarded Stage-3 authority refresh for the independent Mask v4 release.
-- The earlier Shampoo v3->v4 transition remains supported, Scalp Care must remain
-- at v2, captured products are preserved, and authority decisions are reopened.
...
DECLARE
  ...
  -- Keep the migration safe when it is applied immediately before the code
  -- deploy: the old runtime may still request a Shampoo-only v3->v4 refresh
  -- while Mask remains at v3 during that bounded deployment window.
...
BEGIN
  ...
  -- During the migration-first deployment window, the preceding runtime can
  -- only request the already-supported Shampoo v3->v4 transition and still
  -- sends a freshly seeded payload. The server must preserve the persisted
  -- capture state instead of comparing or storing that destructive seed.
  v_preceding_runtime_refresh :=
    v_new_snapshot_versions IS NOT DISTINCT FROM v_previous_current_versions
