-- Correct the already-published legacy normalizer without rewriting migration
-- history. COALESCE is PostgreSQL syntax and cannot be schema-qualified.
...
AS $$
DECLARE
  v_limit integer := pg_catalog.greatest(1, pg_catalog.least(COALESCE(p_limit, 100), 500));
  v_updated integer := 0;
BEGIN
  WITH eligible AS (
    SELECT
