-- Personal Plan search dispositions quarantine exact-readiness exceptions
-- without changing global catalog visibility or recommendation flags.
CREATE TABLE IF NOT EXISTS public.personal_plan_product_search_dispositions (
  ...
  sources jsonb NOT NULL CHECK (
    pg_catalog.jsonb_typeof(sources) = 'array'
    AND pg_catalog.jsonb_array_length(sources) > 0
  ),
  source_batch text NOT NULL CHECK (source_batch ~ '^S5-[0-9]{2}-[a-z0-9-]+$'),
  ...
  CONSTRAINT personal_plan_product_search_dispositions_reason_pair CHECK (
    (
      disposition = 'identity_ambiguous'
      AND reason_code = 'identity_ambiguous'
    )
    OR (
      disposition = 'retired_from_personal_plan'
      AND reason_code IN ('wrong_category', 'duplicate_identity', 'non_hair_product')
    )
    OR (
...
ALTER TABLE public.personal_plan_product_search_dispositions ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.personal_plan_product_search_dispositions FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE ON TABLE public.personal_plan_product_search_dispositions TO service_role;

CREATE INDEX IF NOT EXISTS personal_plan_product_search_dispositions_batch_idx
  ON public.personal_plan_product_search_dispositions(source_batch);
