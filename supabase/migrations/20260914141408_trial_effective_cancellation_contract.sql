-- Subsequent cancellations target the provider-verified current agreement after a restore/switch.
CREATE OR REPLACE FUNCTION public.issue_trial_cancellation_capability(p_enrollment_id uuid, p_user_id uuid)
RETURNS boolean LANGUAGE sql SECURITY INVOKER SET search_path = '' AS $$
  SELECT EXISTS (
    ...
    JOIN public.billing_subscriptions AS b ON b.trial_enrollment_id = e.id
      ...
      AND b.provider_subscription_id = CASE WHEN e.provider='stripe' THEN e.provider_agreement_id ELSE public.read_trial_effective_contract(e.id)->>'provider_agreement_id' END
...
CREATE OR REPLACE FUNCTION public.load_trial_cancellation_provider_operation(p_declaration_id uuid, p_user_id uuid)
...
LANGUAGE sql SECURITY INVOKER SET search_path = '' AS $$
  SELECT e.id, e.user_id, e.provider, b.provider_customer_id,
    (public.read_trial_effective_contract(e.id)->>'provider_agreement_id'), e.original_trial_end_at, o.status, e.cancel_at_period_end,
  ...
  WHERE d.id = p_declaration_id AND d.user_id = p_user_id AND e.user_id = p_user_id
    ...
    AND NOT EXISTS (SELECT 1 FROM private.trial_management_operations m
      WHERE m.enrollment_id=e.id AND m.kind='restore' AND m.status='committed' AND m.completed_at>d.submitted_at);
$$;

CREATE OR REPLACE FUNCTION public.confirm_trial_cancellation_provider_operation(
...
BEGIN
  ...
  WHERE o.declaration_id = p_declaration_id AND d.user_id = p_user_id AND e.user_id = p_user_id
    ...
    AND (public.read_trial_effective_contract(e.id)->>'provider_agreement_id') = p_provider_agreement_id
