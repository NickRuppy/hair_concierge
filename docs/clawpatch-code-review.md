...
Use the current review-map slice names instead of the retired memory/RAG umbrella label: `Chat memory, state, and traces` for runtime state/debug trace work, and `Product matching and catalog chunks` for catalog chunk ingestion or retrieval-eval changes.
