...
3. **Keep/swap decisions (the one interaction):** per category, Nick records the call's outcome — **behalten** (keep the participant's product) or **tauschen** (pick one of the computed alternatives). Persisted and editable **after** the call too: PDF creation is an async curation step, never an automatic call-end artifact. When Nick is done adjusting, he sets an explicit **„Finalisieren"** status (e.g. `call_finalized_at` on `discovery_intakes`); the PDF view is generated/rendered only from that finalized state, and Nick sends it manually (WhatsApp/email by hand — no email service integration).
