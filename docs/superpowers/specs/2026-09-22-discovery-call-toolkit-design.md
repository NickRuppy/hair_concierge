...
3. **Keep/swap decisions (the one interaction):** per routine step (`decision_key`), Nick records the call's outcome — **behalten** (keep the bound intake product) or **tauschen** (pick an alternative; any catalog product id, not restricted to the previews). Persisted in `discovery_call_decisions` and editable **after** the call too: PDF creation is an async curation step, never an automatic call-end artifact. When Nick is done adjusting, he sets an explicit **„Finalisieren"** status (`call_finalized_at` + `finalized_source_hash` on `discovery_intakes`, requires `submitted`); while finalized, decisions are frozen (un-finalize to edit, allowed any time since sending is manual). The PDF view renders only from the finalized state — with a drift warning if the underlying engine output has changed since finalize — and Nick sends it manually (WhatsApp/email by hand — no email service integration). Swap options: alternatives-only vs. free catalog pick is an open ruling for Nick (plan §5).
