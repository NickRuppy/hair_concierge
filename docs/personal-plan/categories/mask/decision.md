...
The category-owned decisions are complete. Final plan-wide function ownership, shared card-level reason salience, and live catalog migration remain deferred shared dependencies, as they are for Conditioner and Leave-in.
...
- `dry_dull_lengths`;
- `hairSurface = rough`;
- `breakage_or_split_ends` until split ends and breakage are separated in the canonical quiz profile;
- `tangling`.
...
6. goals, texture, and elasticity guide target direction but do not count as the strong second signal;
7. chemical treatment contributes once through the shared assessment and may also appear as a user-facing reason, but is not added again as a score;
8. ownership never changes the underlying need tier.
...
Explicit low-volume/weighed-down sensitivity may shift the target one level lighter. Density and length change amount and sectioning rather than formula weight. Damage does not make the target richer; a light Mask may still provide high repair support.
...
| `careDirection` | Same direction | Either target or product is `balanced` | Direct `moisture` versus `protein` opposition when care direction is required |
...
Verified exact-product directions override the category fallback. Store the following Mask-relevant facts through the shared product protocol authority rather than creating a second category-local protocol store:
...
- `optional` Mask uses `cadenceMode = as_needed`;
...
2. the already-known personal wash frequency;
3. the already-known fine/volume/weighed-down sensitivity.
...
Explicit fine/volume/weighed-down sensitivity reduces cadence one step: `high → alle 2 Wochen`, `standard → alle 3 Wochen`. Prefer a lightweight Mask as the first response to fine hair; the cadence reduction provides an additional conservative tolerance adjustment for the explicitly weight-sensitive profile. Wash frequency then places that target on the closest eligible wash without adding washes.
...
6. `mask-high-fine-weight-sensitive`: high need plus explicit fine/volume sensitivity = cadence reduced to every two weeks and light target.
7. `mask-standard-weight-sensitive`: standard need plus explicit weight sensitivity = cadence reduced to every three weeks.
...
- final primary/supporting ownership across Conditioner, Leave-in, Mask, Oil, Bondbuilder, and Styling;
- shared two-to-three-fact card salience and German presentation templates;
- live catalog completeness, verified protocol coverage, and migration/backfill planning.
