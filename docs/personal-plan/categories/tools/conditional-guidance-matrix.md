...
version_note: "v3 (2026-08-25) — counterpart-review reconciliation: volume triggers, the H08/H12/H15 decision-log rows and the checkpoint restated against the ruled semantics."
...
| `tools.heated.volume_set` | eligible `heated_rollers`, `heated_brush`, or `heated_multi_styler` with verified `create_volume`/`set_style` | `volume_balance` whose direction resolves to volume_up (inferred per `D1`, or through the `low_volume_or_weighed_down` override); one approach inside the shared volume/set `basis`; reported viable form is prioritized | selected approach appears as optional post-wash branch on every wash | Heat protection `basis`; contact-mode fallback and exact-product override | verified volume/set capability plus safety-critical fact minimum | one choice member, not another need; selected/acquired product activates exact guidance; ambiguous owned coverage remains conditional | fixtures 6, 47-54; `confirmed` |
...
| `tools.heatless.volume_set` | eligible band, roller, rod, or former with verified `create_volume`/`set_style` | `volume_balance` whose direction resolves to volume_up (inferred per `D1`, or through the `low_volume_or_weighed_down` override); one approach inside the shared volume/set `basis`; reported viable heatless form is prioritized | selected approach appears as optional post-wash branch on every wash | same exact-protocol precedence and tension fallback; clips/ties may be subordinate optional aids, never a second basis requirement | route-critical fact minimum; unknown use state or setup blocks the exact candidate | one choice member, not another need; user selects approach, then product lifecycle owns shopping/acquisition/guidance activation | fixtures 6, 47-49, 53-54; `confirmed` |
...
| `tools.brush.pick_optional` | `hair_pick` with `create_volume` | `concerns` contains `low_volume_or_weighed_down` plus `curly`/`coily` — under `D1` curly and coily resolve to **control**, so the override is the only surviving path to this route; reported form first | optional finishing/root-volume event, not another weekly cadence | lift gently without scraping/pulling; no growth or guaranteed volume claim | type/job and relevant use state only | collapsed optional/lightweight approach; never another shared-volume requirement | fixtures 67, 70; `confirmed` |
...
- **`C01` confirmed, amended 2026-08-25 (`D1` staleness):** this family is never standalone or `basis`. Only four existing parent events may make a securing aid optional: sectioning for selected styling/application; holding/support required by a selected heated/Heatless set; a selected Night Protection method; or root-volume clipping when the volume direction resolves to volume_up (inferred per `D1`, or the `low_volume_or_weighed_down` override). Length, density, texture, catalog presence, or ordinary reported securing alone never create the need.
...
| 2026-08-05 | `H08` | Use only the explicit canonical `volume` goal to create the proactive volume/set need. | `confirmed, reversed 2026-08-24` | Reversed by gate `D1`. The new rule is the amended `H08` entry in the Hitzestyling checklist above: `volume_balance` is directionless, so direction is inferred in-plan from texture and thickness via the ratified `volume-direction.ts` predicate, every inferred route carries `tools.styling.volume_direction_inferred`, and `low_volume_or_weighed_down` triggers the volume routes on its own and overrides the inference. Structural/cosmetic signals and mere ownership remain forbidden as direction signals. |
...
| 2026-08-05 | `H12` | Collect styling-tool ownership family-first, then conditionally drill into the heatless form groups. | `confirmed, amended 2026-08-24` | Amended to **five** groups, adopting the production set: `heatless_curling_band`, `setting_roller`, `foam_roller`, `flexi_rod`, `setting_former`. The earlier four-group list omitted `foam_roller`; it is its own group, not merged into `setting_roller`. The initial screen stays light while route resolution receives recognizable product-type information; users who do not select Heatless see no unnecessary drilldown. |
...
| 2026-08-05 | `H15` | Treat reported Heat-protection use as full dependency coverage. | `confirmed, reversed 2026-08-24` (`D9a`) | The old rule read a two-state legacy boolean no module reads. See the rewritten `H15` text in the Hitzestyling checklist above: coverage is judged **per heat event** from `heatEvents[…].protectionConsistency`, and **only `always` counts as covered** — `sometimes` gets the „mach's konsequent" nudge at tier „empfohlen", `no`/`unsure` get the recommendation, and the portfolio result is the union of the uncovered events. The legacy `uses_heat_protection` boolean stays unread; unsupported exact protection temperatures still remain unstated. |
...
As of 2026-08-24 the nine spec gates `D1`-`D9` plus the second-round rulings `R1`-`R4` are ruled and amended into this tracker, `decision.md`, `input-mapping.md` and `fixtures.md`.

**As of 2026-08-25 the WS4 mockup pass and the user-journey sign-off are DONE** (Nick; mockup evidence `plans/mockups/ws4-2026-08-25/`, recorded in the rulings ledger). That closes the user-facing gate for the `D3a` lead copy — ratified as „Wähle die Bereiche, aus denen du schon Produkte hast. Nicht gewählt = hast du nicht." — the removal of „Nichts davon" from the drying question, and the „Nur Finger" plus Wildschweinborsten-Bürste cards.

What remains is the **WS4 implementation** of those ratified changes (including the label-wrapping fix and the two card images the mockup review flagged). Implementation authorization remains separate from this tracker.

The counterpart-review reconciliation of the four spec documents was applied on 2026-08-25; its findings ledger is in `plans/2026-08-24-hair-tools-d1-d9-rulings.md`.
