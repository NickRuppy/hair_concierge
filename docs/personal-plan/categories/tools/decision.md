...
- **`fulfilledBy`** is either a member whose `coverage.state` is covered, or `null`. It may never name an uncovered member. **Amended 2026-08-25, refined same day (adversarial lane + remediation, entailed by `H10` and the confirmed "prioritize a reported viable route" clause):** a member fulfils the group when it is **`covered_by_report`** (the user reported an eligible form — fulfilment counts once and, where exact capability is unverified, the card uses `H10`'s conditional use-yours wording) **or** covered with `capabilityVerified` true. **Derived, unverified coverage never fulfils** — a plain Föhn projected from the drying behaviour (`A04`/`H10`) leaves the group unfulfilled and neutral while that member renders conditional use-yours. Without this refinement no volume group could be fulfilled in Phase 1 at all (nothing sets `capabilityVerified` before the Phase-2 catalog), which would put a redundant neutral card beside a reported Warmluftbürste's own card.
