...
Heat protection is binary only; degrees cut over 2026-09-14 (AD-6). Legacy 221–232 °C figures on existing rows were marketing use-condition parameters, not measured protection levels; `heat_protection_max_c` is no longer emitted by this adapter, no longer read by the ranking/catalog-facts/application layers, and a migration has nulled the legacy column values.
