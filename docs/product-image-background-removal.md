...
## Batch runner (T4b, `scripts/product-images/`)

For a batch of products (the ~30-SKU pilot, later ~150-200), running the
decision tree above by hand per image doesn't scale. `batch-run.ts` automates
the parts of the tree that are mechanically decidable, and hands the rest to
a human via a contact sheet:

| Script                    | Purpose                                                                  |
| -------------------------- | ------------------------------------------------------------------------- |
| `batch-run.ts`             | Runs the automated decision tree over a list of `{id, source}` images     |
| `contact-sheet.ts`         | Builds a local `review.html` from `batch-run.ts`'s `results.json`         |
| `finalize-approved.ts`     | Locally finalizes approved cutouts via the existing product-intake asset pipeline (no Supabase writes) |

### What's automated vs. what needs a human

`batch-run.ts` follows the tree as far as it can decide without eyes on the
image:

1. **Source already has alpha** → passthrough (most common path for
   brand/retailer assets, per Step 0 above).
2. **No alpha** → Vision (`removebg.swift`). "No subject found" or a
   degenerate alpha coverage (near-zero, i.e. nothing survived) → padded
   retry (`removebg-padded.swift`).
3. **Boundary halo heuristic** — a density-of-warm-dark-pixels score
   computed on the ring of pixels just inside the alpha silhouette's outer
   edge (same geometric signal `remove-baked-shadow.py` uses: cast shadow =
   dark pixels connected to the boundary). Above a hard threshold, it
   actively runs `remove-baked-shadow.py` and keeps the result only if the
   halo score actually improved. Above a softer threshold (or if no
   fixable improvement is found, or if there's no python interpreter with
   numpy/scipy/PIL available), the image is **flagged**, not silently
   "fixed" — false positives at the boundary heuristic level are expected
   and cheap to review on the contact sheet.

What batch-run.ts explicitly does **not** decide automatically (per the doc
above, still requires a human on the contact sheet):

- Sachet/box artwork with a person in the printed art (Vision lifting the
  person out of the packaging) — this needs a human to recognize "the
  product is the full rectangle" and reject the cutout.
- The vivid-color-product BiRefNet path (Step 4 exception) — not wired; a
  vividly colored product with a baked shadow will be flagged, not silently
  mis-processed.
- Whether an "ok" passthrough or Vision cutout is actually clean — the
  contact sheet's magenta composite is there so a human can eyeball every
  image, `ok` is a triage default (pre-checked), not a guarantee.

Status meanings in `results.json`:

- `ok` — no automated signal fired; pre-checked on the contact sheet.
- `flagged` — a heuristic fired (borderline/likely halo, deshadow
  attempted-but-not-conclusive, deshadow skipped for lack of a python
  interpreter); unchecked by default, reason recorded.
- `failed` — no usable cutout could be produced at all (no subject found
  even with padding, or degenerate alpha coverage); no checkbox to approve.

### Running it

```bash
# 1. Batch cutout — items.json is [{ "id": "...", "source": "path-or-URL" }, ...]
npm run products:images:batch -- --input items.json --out /tmp/batch-out

# 2. Build the review sheet, open it locally, check/uncheck, download approved-ids.json
npm run products:images:contact-sheet -- --results /tmp/batch-out/results.json
open /tmp/batch-out/review.html

# 3. Finalize the approved images locally (no Supabase writes — pure sharp/crypto)
npm run products:images:finalize-approved -- \
  --results /tmp/batch-out/results.json \
  --approved /tmp/batch-out/approved-ids.json \
  --out /tmp/batch-out/finalized

# 4. Upload one product at a time — a human runs this explicitly, per product,
#    with real Supabase credentials. Nothing above writes to Storage or prod.
npx tsx scripts/product-intake/upload-package-image.ts \
  --package /tmp/batch-out/finalized/<id> --apply --confirm
```

`remove-baked-shadow.py` needs a python3 with `numpy`/`scipy`/`PIL` (the
Step 3 rembg venv works). Pass `--python <path>` to `batch-run.ts`, or set
`PRODUCT_IMAGE_PYTHON`; otherwise it checks `/tmp/rembg-venv/bin/python3` and
degrades gracefully (flags instead of fixing) if none is found.

Sequential processing (one image at a time); each Vision/padded call is a
fresh `swift <script>.swift` invocation (~1-2s of compile/startup overhead
per call is normal — see the pilot-30 smoke-test timing recorded in
`plans/2026-09-01-scan-db-expansion-pilot.md`'s T4b notes).
