# Conditioner Research and Classification Standard v1.6

Status: locked reusable nine-property research authority
Version: 1.6
Logic lock: approved by Nick on 2026-09-02 after reviewing the protein/medium, balanced/medium, and moisture/high anchor patterns
Scope: conventional short-contact rinse-out conditioners, Germany/EU
Normative source: this Markdown file

This lock fixes the comparison vocabulary, thresholds, evidence ceilings, and reasoning contract for future Conditioner ingredient research. It does not approve every pilot product, diagnose a user's protein/moisture state, activate catalog fields, or authorize production use. Conditioner Production Adapter v1 may derive today's compatibility rows only after this research is complete; review and publication remain separate gates.
...
- `damage_fit`: low conditioning maps to exactly `healthy`; moderate conditioning and general high conditioning without a qualifying specialist route map to exactly `healthy` plus `moderately_damaged`. High conditioning with (a) a distinct protein, peptide, or keratin fibre-film route, (b) named bond chemistry, (c) exceptional corroborated protection, or (d) a relevant exact-product test maps to exactly `moderately_damaged` plus `highly_damaged`—it replaces the general-high set rather than appending a third value. Generic silicone, oil, panthenol, ceramide, cationic polymer, repair naming, or a generic lubrication candidate alone does not qualify. This is a broad product prior, not a repair-efficacy claim.
...
Current live `protein_moisture_balance`, `repair_level`, weight, and ingredient flags are comparison-only historical data. They cannot determine a new classification or break a tie. After a new product's complete profile is established, Conditioner Production Adapter v1 may derive the current compatibility direction from `care_direction`: moisture → `snaps`, balanced → `stretches_bounces`, and protein → `stretches_stays`. This is a temporary production matching policy, not an ingredient observation or proof of a user deficiency. The full research record remains authoritative and the adapted rows remain separately reviewable.
...
Systemic rule changes require pilot rerun. Product-specific uncertainty remains uncertainty. The v1.5 Damage Fit comparison is historical provenance. Independent Reviewer G completed `care_direction` and `repair_support_level` with 22/22 exact agreement against the accepted key under the retained v1.6-rc1 source snapshot. Promotion to final v1.6 locked those rules and clarified the already-accepted two-value Damage Fit output shape; it changed no accepted product classification. The active 94/99 pre-adjudication, 85/99 post-adjudication, and 68/77 non-focus composite preserves frozen Reviewer F values for the earlier seven fields and appends Reviewer G only for the new two; it is not a fresh de-novo nine-property blind rerun and cannot establish full v1.6 repeatability.
