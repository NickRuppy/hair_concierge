...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shampoo                | Derive the union from reviewed bucket rows: every `schuppen` row contributes `shampoo_dandruff`; every non-`schuppen` row contributes `shampoo_everyday`. A treatment-only Shampoo is complete without `shampoo_everyday`; a verified dual-use Shampoo requires both. |
...
- If heat protection is claimed, store it as
  `provides_heat_protection: true`. Binary only; degrees cut over 2026-09-14
  (AD-6) — do not store a degree value or a combined sentence.
...
- Application protocols must match the roles derived from these reviewed bucket
  rows exactly. Do not add `shampoo_everyday` merely to complete a Shampoo
  category payload. If the exact source supports ordinary or daily use, first
  research the matching non-`schuppen` scalp-route facts; daily-use suitability
  does not itself choose a bucket or instruct the Personal Plan to wash daily.
