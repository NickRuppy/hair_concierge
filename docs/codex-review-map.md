...
| Chat memory, state, and traces | `src/lib/chat-runtime/`, `src/app/api/memory/`, `src/app/api/chat/` | Memory isolation, conversation-state migration, trace redaction, title + memory side effects | `tests/user-memory.spec.ts`, `tests/conversation-state.spec.ts`, `tests/chat-debug-trace.spec.ts`, `tests/agent-v2-production-chat-pipeline.spec.ts` |
| Product matching and legacy catalog chunks | `src/lib/product-matching/`, guarded `scripts/ingest-product-chunks.ts`, `scripts/eval-retrieval.ts` | Product matching determinism, guarded legacy chunk metadata, retrieval eval regressions | `tests/product-matcher.spec.ts`, `tests/product-list-chunks.test.ts`, `npm run test:retrieval:ci` |
...
| Chat memory, state, and traces | 8 | 3 library, 5 route | 6 | `feat_library_8b23ea7afc` Node source `src/app/api/memory`; `feat_library_931e94f82f` Node source `src/lib/chat-runtime`; `feat_library_df6bcacba8` Node source `src/app/api/chat` |
| Product matching and legacy catalog chunks | 3 | 3 library | 1 | `feat_library_32aedee59d` Node source `src/lib/product-matching`; `feat_library_32b0f83226` Node source scripts; `feat_library_d50a235b25` Node source scripts ingestion |
