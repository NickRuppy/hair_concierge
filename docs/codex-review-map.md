...
| Agentic chat and tools | `src/app/api/chat/`, `src/lib/agent/`, `src/components/chat/`, `src/hooks/use-chat.ts` | Tool routing, product selection, answer composition, visible failure handling, German response UX | `npm run test:agent`, `npm run test:chat:ci`, selected Playwright chat specs |
| RAG and memory | `src/lib/rag/`, `src/app/api/memory/`, `scripts/eval-retrieval.ts` | Retrieval thresholds, memory extraction, clarification state, source grounding | `npm run test:retrieval:ci`, `tests/conversation-state.spec.ts`, `tests/user-memory.spec.ts` |
...
| RAG and memory | 11 | 9 library, 2 route | 5 | `feat_library_26b3ce8594` source group `src/lib/rag/retrieval`; `feat_route_4ccd39b375` Route `/api/memory`; `feat_route_6355863e58` Route `/api/memory/:id` |
